...
- DB는 시작 시와 매일 백업합니다. 완료된 새 DB 백업을 감지하면 영상을 별도 호스트 폴더에 증분 백업하고 manifest에 대응 DB 백업 이름을 기록하며 기존 백업 파일은 삭제하지 않습니다. 이 백업도 같은 Mac에 있으므로 기기 자체 손실 대비에는 외부 디스크/원격 복제가 추가로 필요합니다.

...
- 자막 템플릿 글꼴: Google Fonts 저장소의 OFL 한국어 글꼴 27종, Galmuri(OFL) 도트 글꼴 2종, 눈누 저장소의 잘난체·카페24 써라운드·심플해·지마켓 산스(각사 무료 라이선스, WOFF→TTF 변환)와 Pretendard·Wanted Sans(OFL)를 `scripts/fetch_fonts.py`가 커밋 해시 고정 URL에서 받아 체크섬을 확인하고 워커 이미지에 설치합니다. 목록·라이선스 링크는 `pipeline/subtitle_fonts.py`, 사용법은 [자막 파일·템플릿 도구](SUBTITLE_TOOL.md#글꼴)입니다.

...
- 정렬 오차(whisper small, 합성 음성): 자막 1 **0.02초**, 자막 2 **0.00초**. 대본의 줄이 자막 경계로 그대로 남습니다.
  ...
  - 단어 시각은 무음 안쪽으로 당겨지기도 합니다(측정: 마지막 문장이 1.73초 이르게 시작, tiny·small 모두). `snap_starts()`가 자막 시작을 가까운 발화 시작(FFmpeg silencedetect)으로 맞춰 무음에서 자막이 먼저 뜨는 것을 막습니다. 앞 자막을 침범하거나 자막이 사라질 만큼 옮겨야 하면 그대로 둡니다.

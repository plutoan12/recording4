...
- 정렬 오차(줄바꿈 대본, `original_split` 적용): 자막 1 **1.00초**, 자막 2 **0.19초**, 자막 3 **0.45초**. 문장 경계 3/3개를 자막 경계로 남겼습니다.
  - **첫 자막만 1초 늦습니다.** 한계값과 같아 간신히 통과한 값이고, 줄 유지를 켜기 전(한 줄 대본) 첫 문장이 0.98초로 잡혔던 것과 비교하면 뒤로 밀린 것입니다. 원인은 확인하지 않았습니다. 앞에 넣은 1초 무음 처리이거나 합성 음성 첫 발화의 불명확한 시작으로 보입니다. 실제 사람 목소리에서 같은 현상이 나는지는 미확인입니다.

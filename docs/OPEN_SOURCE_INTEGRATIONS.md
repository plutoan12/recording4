...
- 자막 템플릿 글꼴: Google Fonts 저장소의 OFL 한국어 글꼴 27종, Galmuri(OFL) 도트 글꼴 2종, 눈누 저장소의 잘난체·카페24 써라운드·심플해·지마켓 산스·에스코어 드림·나눔스퀘어라운드·티몬 몬소리·스웨거·SUIT·배민 한나 Pro·을지로오래오래·온글잎 무궁·메이플스토리·넥슨 Lv1 고딕(각사 무료 라이선스 또는 OFL, WOFF/WOFF2→TTF/OTF 변환)와 Pretendard·Wanted Sans(OFL), 그리고 컬러 이모지용 Twemoji Mozilla(COLRv0, CC-BY-4.0 그림 / MIT 코드, 릴리스 태그 고정)를 `scripts/fetch_fonts.py`가 고정 URL에서 받아 체크섬을 확인하고 워커 이미지에 설치합니다. Twemoji는 색 층을 겹쳐 그리는 `R4 Color Emoji` 글꼴과 표로 변환합니다(`pipeline/subtitle_emoji.py`). 영상에 Twemoji 출처(CC-BY) 표기가 필요합니다. 목록·라이선스 링크는 `pipeline/subtitle_fonts.py`, 사용법은 [자막 파일·템플릿 도구](SUBTITLE_TOOL.md#글꼴)입니다.
- `/subtitle-templates`, `/clips`의 `subtitle_template`: 영상에 굽는 자막 모양(글꼴·색·외곽선·위치)을 내장 템플릿 이름으로 고릅니다. 구현은 `pipeline/subtitle_templates.py`이며 pysubs2 스타일로 바꿔 렌더의 ASS에 넣습니다. `/subtitle-animations`, `/clips`의 `subtitle_animation`: 자막 움직임(팝·바운스·슬라이드·페이드·타자기·단어별 등장·노래방 강조 등)을 libass의 ASS 애니메이션 명령(`\t`, `\move`, `\fad`)으로 만듭니다(`pipeline/subtitle_motion.py`). 별도 모션 그래픽 라이브러리는 쓰지 않습니다. 편집기의 정확 미리보기는 [jassub](https://github.com/ThaUnknown/jassub)(libass WASM, MIT, npm `jassub`)으로 같은 ASS를 브라우저에서 그리고, `POST /subtitle-preview`·`GET /subtitle-fonts/{family}`가 ASS와 글꼴을 내려줍니다. 같은 템플릿을 파일에 적용하는 명령줄은 [자막 파일·템플릿 도구](SUBTITLE_TOOL.md)입니다.

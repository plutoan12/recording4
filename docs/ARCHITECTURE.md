...
| transcript_segments | 원본 ID, 화자, 겹침 표시, 시작·종료, 원문, 생성한 STT 단계 실행 ID |
...
같은 새 버전에 **겹침 표시**를 함께 찍습니다. 화자 분리가 준 구간에서 서로 다른 화자 둘 이상이 동시에 말한 시간(`pipeline.overlap.overlap_regions`)을 구하고, 자막 길이의 20% 이상이 그 시간에 걸치면 표시합니다. 표시는 **믿을 수 없다는 알림**이지 지우라는 뜻이 아닙니다. 겹말에서 전사가 무너지는 것은 실측으로 확인했고(CER 92~116%) 전사를 고치는 길은 아직 없습니다. `overlap`이 비어 있으면(NULL) **아직 안 재 본 것**이고 "겹치지 않음"이 아닙니다. 사람이 대본을 고쳐 만든 새 버전은 시각이 바뀔 수 있어 표시를 물려받지 않고 다시 재기 전까지 비어 있습니다.

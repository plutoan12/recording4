...
- 허용 오차: 세그먼트 음성 길이가 원본 구간 대비 ±10% 이내면 그대로 사용합니다.
...
- `budgets`에 월 한도와 작업당 한도를 둡니다. 유료 호출 전에 단계 비용을 추정하고 누적액과 비교합니다.
- 한도를 넘으면 호출하지 않고 작업을 `blocked`로 둡니다. 사람이 한도를 올리거나 작업을 취소해야 진행됩니다.
...
- 새 게시가 성공한 것을 조회로 확인한 뒤 이전 영상을 비공개로 전환하고 이전 publication을 `superseded`로 둡니다. 순서를 바꾸면 공백이 생깁니다.

...
| `quality FILE [--shape] [--json]` | 자막 품질 요약(장수·표시 시간·읽기 속도·규칙 위반 비율) | 0 |
...
```text
--language ko|en   --pacing broadcast|shortform
--max-chars 16   --max-lines 2   --max-cps 12   --min-duration 1   --max-duration 7
```
...
같은 글자라도 **어디서 끊느냐**로 인상이 크게 달라집니다. 방송 자막은 한 장을 길게 보여 주고, 숏폼은 말에 맞춰 잘게 끊습니다. 편집기의 **자막 끊기** 선택(`/clips`의 `subtitle_pacing`), 명령줄 `--pacing`으로 고릅니다.

| | `broadcast` (기본) | `shortform` |
|---|---|---|
| 줄당 글자 폭 | 16 (한국어 지침) | 11 |
| 줄 수 | 2 | 1 |
| 표시 시간 | 1~7초 | 0.6~2.5초 |
| 읽기 속도 한도 | 초당 12자 | 초당 16자 |
| 끊는 기준 | 글자 수와 문장 | **말한 시각**(있으면) |

기본값은 그대로 둡니다. 고르지 않으면 지금까지와 똑같이 동작하므로 이미 만든 편집본의 렌더 결과가 바뀌지 않습니다.

`shortform`은 대본의 단어 시각(`Cue.words`)이 있으면 **말한 자리에서** 끊습니다. 글자 수로 나누면 말 중간에서 자막이 바뀌지만, 단어 시각으로 나누면 자막이 말과 같이 바뀝니다. 단어 시각이 없거나(자막 파일을 그대로 읽은 경우) 사람이 글자를 고쳐 단어와 맞지 않으면 글자 수로 나누는 방식으로 조용히 돌아갑니다.

끊을 때 두 가지를 더 봅니다.

- 뒤 말을 꾸미는 짧은 말(그, 이, 안, 못, 잘, 좀, 더 …)이 자막 끝에 혼자 남지 않게 다음 자막으로 넘깁니다. "말했던 그 / 영상"이 아니라 "말했던 / 그 영상"이 됩니다.
- 자막 끝을 다음 자막이 시작할 때까지 최대 0.4초 띄워 둡니다. 말 사이가 조금 빌 때 자막이 깜빡이지 않습니다.

시간이 모자라면(표시 시간이 최소 시간보다 짧아지면) 더 나누지 않으므로 한 줄 규칙이어도 두 줄이 남을 수 있습니다. 그런 자막은 `quality`와 `check`가 위반으로 보고합니다.

### 품질 재기

규칙을 바꾸면 좋아졌는지 눈으로만 보지 말고 숫자로 견줍니다.

```bash
r4-subtitles quality in.srt --language ko                        # 지금 상태
r4-subtitles quality in.srt --pacing shortform --shape           # 숏폼으로 다시 끊은 뒤
```

자막 장수, 표시 시간·글자 폭·읽기 속도·자막 사이의 최소·가운데·평균·최대, 화면에 자막이 떠 있는 시간 비율, 규칙 위반 비율을 냅니다. `--json`으로 받아 기록해 두면 전후 비교가 됩니다.

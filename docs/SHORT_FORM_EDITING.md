...
자막 모양은 `subtitle_template`(내장 템플릿 이름)으로, 움직임은 `subtitle_animation`(비우면 템플릿 값, `none`이면 없음)으로 고릅니다. 템플릿은 굽는 자막의 글꼴·색·외곽선·위치·움직임만 정하고 줄바꿈·분할은 표시 규칙이 정합니다. 목록과 값은 [자막 파일·템플릿 도구](SUBTITLE_TOOL.md)에 있습니다.

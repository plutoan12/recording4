...
## 여러 구간 이어 붙이기·배속·페이드·배경음악 (2026-09-21)

`EditSpec.segments`가 비어 있으면 지금까지처럼 `start~end`를 통째로 씁니다. 구간을 고르면
그것들만 순서대로 이어 붙입니다.

| 설정 | 뜻 |
|---|---|
| `segments[].start/end` | 이어 붙일 원본 구간. 시작~끝 안에, 시간 순서대로, 겹치지 않게 |
| `segments[].speed` | 구간별 배속(0.5~4.0). 소리도 `atempo`로 같이 빠르게 합니다 |
| `fade_in` / `fade_out` | 결과 기준 시작·끝 페이드(초). 영상과 소리에 함께 겁니다 |
| `music_asset_id` | 배경음악으로 쓸 **검사 통과한 원본**. 그 원본의 소리만 가져옵니다 |
| `music_gain_db` | 배경음악 음량(기본 -18dB). 잰 값이 아니라 정한 값입니다 |
| `music_duck` | 말할 때 배경음악만 낮춥니다(`sidechaincompress`) |

**길이 상한은 결과에만 겁니다.** 열 시간짜리 원본에서 세 조각을 고르는 것이 이 기능의
요점이라, `end - start`가 아니라 이어 붙인 길이가 180초 이하인지 봅니다. 자막은
`concat_cues`가 이어 붙인 시간축으로 옮기고, 빠진 구간의 자막은 함께 빠집니다.

FFmpeg 쪽은 `trim`/`atrim` → `concat` → 화면 맞추기 → 자막 → 페이드 → 배경음악 섞기
순서입니다. 아무것도 고르지 않은 편집은 **예전 명령 그대로** 나갑니다. 결과가 달라지지
않게 하기 위해서입니다. 배경음악은 `amix=normalize=0`으로 섞습니다. 기본값이면 입력 수만큼
나눠 말소리가 절반이 됩니다.

## 무음 빼고 남길 구간 제안 (media task `silence`)

말이 없는 구간을 빼고 남길 구간을 **제안**합니다. 적용은 사람이 편집 화면에서 누릅니다.
발화 구간은 VAD(`worker.analysis.vad_spans`)가 찾고, 그 위에 auto-editor에서 가져온
여백(`margin`)과 다듬기(`min_cut`·`min_clip`)를 겁니다(`pipeline.cuts`).

기본값은 여백 0.2초, 가장 짧은 잘림 0.5초, 가장 짧은 남김 0.4초입니다. **잰 값이 아니라
정한 값입니다.** 실제 영상으로 다시 맞춰야 합니다.

## 편집 설정 한 장 미리보기 (media task `preview`)

전체를 합성하지 않고 결과의 한 순간만 PNG로 뽑습니다. 결과 시각을 원본 시각으로 되돌려
(`source_time`) 그 자리만 읽고, 그 순간에 떠 있는 자막만 0초로 옮겨 그립니다. 렌더와
**같은 `EditSpec`**을 쓰므로 미리본 것과 결과가 갈라지지 않습니다.

## 이음매 전환 (2026-09-21 추가)

구간을 딱 붙이는 대신 겹쳐서 이을 수 있습니다(FFmpeg 내장 `xfade`, 소리는
`acrossfade`). `EditSpec.transition`에서 고릅니다(비우면 지금까지처럼 딱
붙이기). 12종만 엽니다 — FFmpeg에는 50여 종이 있지만 화려한 것은 숏폼에서
산만합니다.

- 영상이 **(이음매 수 × 겹침)만큼 짧아집니다.** `output_seconds`가 그 값을
  빼고 세고, `-t`도 그 길이로 잡습니다.
- 겹침은 **가장 짧은 구간의 절반**까지입니다. xfade가 그보다 길면 거부합니다.
  그래도 짧으면 전환을 포기하고 딱 붙입니다. 탐지 결과 때문에 렌더가 실패하면
  사람이 고칠 방법이 없습니다.
- 구간이 하나면 이을 자리가 없으므로 전환도 없습니다.

GL Transitions는 직접 빌드해야 하는 외부 필터라 쓰지 않았습니다.

## 자동 리프레이밍 (`EditSpec.reframe`)

세로로 자를 때 가로 중심이 `focus_x` **고정 숫자**였습니다. 이제 얼굴을 따라
움직일 수 있습니다. `mode="crop"`에서만 돕니다(`pad`는 화면 전체를 남기므로
자를 것이 없습니다).

| 값 | 기본 | 뜻 |
|---|---|---|
| `deadzone` | 0.06 | 이보다 작게 움직이면 화면을 움직이지 않습니다(고개 까딱임) |
| `max_speed` | 0.25 | 초당 따라갈 수 있는 거리(화면 폭 비율) |

검출 결과를 그대로 따라가면 **화면이 덜덜 떨립니다.** 그래서 작은 움직임은
무시하고 속도를 제한하며, 검출이 없는 구간은 마지막 위치를 유지합니다
(`pipeline/reframe.py`). 얼굴 시각은 **이어 붙인 뒤의 시간축**으로 옮깁니다
(`crop`은 `trim`·`concat` 뒤에 옵니다). 검출기가 얼굴을 못 찾으면 빈 경로이고
crop은 지금까지대로 `focus_x` 고정입니다.

- 사람이 여럿이면 **가장 큰 얼굴**만 따라갑니다(누가 말하는지는 모릅니다).
- 세로(y)는 건드리지 않습니다. 16:9 → 9:16에서는 얻는 것이 적고 위아래로
  흔들리면 더 눈에 띕니다.

## 음성 잡음 제거 (`EditSpec.denoise`)

FFmpeg 내장 `afftdn`입니다. 새 의존성이 없습니다. `soft`는 잡음 바닥 -20dB,
`strong`은 -35dB로 더 깎지만 목소리도 같이 깎일 수 있습니다. 비우면 건드리지
않습니다. 구간을 이어 붙인 **뒤에** 겁니다.

...
## 겹쳐 말한 자막 표시 (media task `diarize`)

편집 화면의 「화자 분리·겹침 표시」로 부릅니다. 화자 분리가 최신 대본의 글자를 그대로 둔
새 버전을 만들면서 **누가 말했는지**와 **그때 다른 사람도 말하고 있었는지**를 함께 찍습니다
(`pipeline.overlap`). 자막 길이의 20% 이상이 두 사람이 동시에 말한 시간에 걸치면 표시가
붙습니다.

표시는 **믿을 수 없다는 알림**입니다. 자막을 지우지도 번역에서 빼지도 않습니다. 겹말에서
전사가 무너지는 것은 실측으로 확인했지만(CER 92~116%) 고치는 길은 아직 없습니다. 표시가
없는 것과 아직 안 재 본 것은 다릅니다 — `/speakers`의 `overlap.checked`로 구분합니다.
사람이 대본을 고쳐 새 버전을 만들면 시각이 바뀔 수 있어 표시는 다시 재기 전까지 비어
있습니다.

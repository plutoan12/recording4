...
**영어 줄 길이는 지침이 아니라 실측에서 나왔습니다.** 처음에 지침대로 42자(폭 21)를 넣었더니 CI 측정이 잡았습니다: 가장 넓은 글자 M 42개가 **1068px**인데 쓸 수 있는 폭은 980px입니다. 지침 42자는 가로 화면 기준이고 우리는 세로 숏폼에 글자 크기 64를 씁니다. 실측으로 들어가는 **38자(폭 19)**로 두고, 읽기 속도는 지침대로 초당 20자(폭 10.0)입니다. 한국어에서 지침 16자를 측정(20자까지 들어감) 안에서 쓴 것과 같은 방식입니다. 측정기는 이제 영어도 글자 수를 늘려 가며 표로 찍습니다. 번역한 자막이면 목표 언어, 원본 대본 그대로면 원본 언어입니다.

...
- 3단계 분석 사본 stable-ts + WhisperX 음소 정렬: 3,769/112/508. 오배정 3자 증가로 기본 적용 제외. 4단계 SepFormer WHAMR + ECAPA 목소리 + 독립 ASR + 두 정렬 확인까지 실제 실행했지만 3단계 대비 추가 회복 없음.
- 3·4단계는 `worker.speaker_recovery`와 `scripts/verify_overlap_review.py --stage 3|4` 실험 경로로 보관. 운영 Celery는 호출하지 않음. 모든 경로는 기존 배정을 유지하며 미확인만 회복 후보로 평가.

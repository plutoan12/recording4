...
분리는 끝까지 돌았는데 **결과를 쓰지 못했습니다.** 표본 폴더는 바깥에서 붙여 준 자리이고 워커 이미지는 uid 10001로 도는데 그 폴더는 러너가 만들었습니다. 읽기만 하면 되는 자리에 쓰려 한 것이 잘못이었습니다. 이제 결과는 임시 폴더에 씁니다. 두 숫자는 다음 실행이 줍니다.
...
`scripts/verify_faces.py`는 **오검출만** 잽니다. 얼굴이 없는 영상(움직이는 도형, 복잡한 무늬)을 넣고 "못 찾았다"가 나오는지 봅니다. 무늬를 얼굴로 착각하는 검출기는 화면을 엉뚱한 곳으로 옮깁니다. 이 점검은 라벨 없이 매번 돕니다.

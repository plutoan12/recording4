...
## 더빙이 배경음을 지우던 것 (2026-09-19)

- 사용자 요청: 미구현 기능 세 가지를 차례대로. 그중 첫 번째(음원 분리). 브랜치 `claude/claude-md-design-review-v8qdz4`.
- 담당 파일: `services/worker/worker/{separation.py(신규),composition.py,workflow_tasks.py}`, `services/api/adminapi/config.py`, `scripts/verify_separation.py`(신규), `tests/test_separation.py`(신규), `.github/workflows/ci.yml`, 문서 넷.

### 무엇이 문제였나

`compose_dub`가 `-map 1:a:0`으로 **원본 오디오를 통째로 버립니다.** 말만 바뀌는 게 아니라 음악과 효과음도 같이 사라집니다. 음악 위에서 말하는 영상을 더빙하면 말만 남은 영상이 나옵니다. 코드 주석에도 "background separation is deliberately not implied"라고 적혀 있었습니다.

### 도구 선택

torchaudio가 싣고 있는 **Hybrid Demucs**(`torchaudio.pipelines.HDEMUCS_HIGH_MUSDB_PLUS`)를 씁니다.

PyPI의 `demucs` 4.0.1은 `torchaudio>=0.8,<2.1`을 요구하는데 워커 이미지는 **2.8.0**입니다. 함께 설치되지 않습니다. torchaudio 번들은 이미 있는 것이라 의존성이 늘지 않습니다. 대신 **번들이 빠진 torchaudio에서는 못 씁니다.** 그래서 CI의 임포트 확인이 매번 존재를 확인하고, 없으면 거기서 실패합니다. 조용히 배경음 없는 결과를 내보내지 않습니다.

### 어떻게 도는가

1. `R4_BACKGROUND_AUDIO_ENABLED`(기본 **꺼짐**)를 켜면 합성 단계가 원본을 목소리/나머지로 나누고 나머지만 남깁니다.
2. `R4_BACKGROUND_GAIN_DB`(기본 **-9dB**)만큼 줄여 대사 아래에 깝니다.
3. `amix`는 `normalize=0`으로 씁니다. **기본값이면 입력 수만큼 음량을 나눠 대사가 절반이 됩니다.**
4. 분리를 못 하면 더빙은 그대로 가고 배경음만 빠집니다. **왜 없는지는 로그에 남깁니다.** 조용히 넘어가면 설정을 켜 놓고도 이유를 알 수 없습니다.

느립니다. CPU에서 1분짜리 소리에 수십 초가 걸립니다. 기본을 꺼 둔 이유입니다.

### 무엇을 재는가

**분리는 근사입니다.** 목소리가 배경에 남고 배경음도 일부 깎입니다. "배경음을 보존한다"는 말은 수치 없이는 하면 안 됩니다.

`scripts/verify_separation.py`가 **아는 두 소리를 섞어** 잽니다. 합성 음성(espeak-ng)과 440Hz 순음을 따로 만들어 섞고, 분리 결과에서 순음 칸의 힘과 그 밖의 힘을 나눠 봅니다. 순음은 한 주파수에만 있어 세기 쉽고, 목소리는 그 칸 밖에 퍼져 있습니다.

- **배경음 남은 비율** (한계 0.50 이상): 음악을 살리려고 켠 설정이 음악을 지우면 안 됩니다.
- **목소리 남은 비율** (한계 0.60 이하): 더빙 대사와 원본 목소리가 겹쳐 들리면 안 됩니다.

한계는 **품질 목표가 아니라 회귀 감시용**입니다. 표본이 하나라 값은 흔들립니다. 합성 음성은 사람 목소리보다 불리한 조건이기도 합니다.

표본은 러너에서 만들고 분리는 워커 이미지 안에서 돕니다. 합성 음성이 이미지에 없기 때문이고, 정렬 검증이 둘로 나뉜 것과 같은 이유입니다.

네 번 연달아 재기 전에 죽은 끝에 **2026-09-19에 처음 쟀습니다.** 막힌 자리를 차례로 적어 둡니다. 세 번째는 이것이었습니다:

```
soundfile.LibsndfileError: Error opening '/audio/background.wav': System error.
```

분리는 끝까지 돌았는데 **결과를 쓰지 못했습니다.** 표본 폴더는 바깥에서 붙여 준 자리이고 워커 이미지는 uid 10001로 도는데 그 폴더는 러너가 만들었습니다. 읽기만 하면 되는 자리에 쓰려 한 것이 잘못이었습니다. 이제 결과는 임시 폴더에 씁니다.

네 번째는 쓰기까지 하고 **읽는 데서** 죽었습니다:

```
wave.Error: unknown format: 3
```

torchaudio는 분리 결과를 **32비트 실수** WAV로 씁니다. 표준 `wave`는 정수만 읽습니다. 재는 쪽이 16비트만 읽게 되어 있던 것이 잘못입니다. soundfile이 있으면 그쪽으로 읽습니다. 실수 WAV와 정수 WAV를 같은 신호로 만들어 읽어 보니 최댓값·주파수 분포가 같았습니다(로컬 확인).

### 첫 실측 (2026-09-19, CI)

```
섞은 소리 4.0초, 44100Hz
  배경음(440Hz 순음) 남은 비율: 0.94 (한계 0.50 이상)
  그 밖의 소리(목소리) 남은 비율: 0.00 (한계 0.60 이하)
```

한계 안에 들어옵니다. **다만 이 표본은 분리에 유리합니다.** 배경음이 440Hz 순음 하나이고 목소리는 합성 음성입니다. 진짜 음악은 목소리와 같은 대역에 겹쳐 있어 이만큼 깨끗하게 갈리지 않습니다. 0.94와 0.00은 "분리가 돌고 있다"와 "예전과 크게 달라지지 않았다"까지만 말합니다. **음질은 사람이 들어야 압니다.**

표본이 6초가 아니라 4.0초인 것은 합성 음성이 그보다 짧고 섞을 때 짧은 쪽에 맞추기 때문입니다.

### 검증

- `pytest -q`: **321 통과 / 6 skip**(새 테스트 7개).
- 그중 하나는 실제로 FFmpeg로 합성해 **배경음이 최종 오디오까지 도달하는지 음량으로 확인**합니다(`volumedetect`). 섞었다고 적어 놓고 안 섞일 수 있습니다.
- 분리를 못 할 때 더빙이 막히지 않는 것도 시험으로 고정했습니다.
- `ruff check`·`ruff format --check` 통과.

### 아직 모르는 것

- **실측값은 없습니다.** 이 커밋의 CI가 처음 냅니다.
- **torchaudio 2.8에 그 번들이 있는지 확인하지 못했습니다.** 이 세션에는 torch가 없습니다. CI 임포트 확인이 답을 줍니다. 없으면 분리 방법부터 다시 정해야 합니다.
- 실제 음악·대사 표본으로는 재지 않았습니다. 순음은 분리기에 유리한 조건입니다.
- 배경음 크기(-9dB)는 **측정에서 나온 값이 아닙니다.** 들어 보고 정해야 합니다.

## 세로로 자를 때 어디를 남길지 제안 (2026-09-19)

- 사용자 요청: 미구현 기능 세 가지 중 두 번째(자동 얼굴 추적). 브랜치 `claude/claude-md-design-review-v8qdz4`.
- 담당 파일: `packages/pipeline/pipeline/framing.py`(신규), `services/worker/worker/{faces.py(신규),media_tasks.py}`, `services/api/adminapi/routers/editing.py`, `apps/web/src/ClipEditor.tsx`, `scripts/verify_faces.py`(신규), `tests/test_framing.py`(신규), `.github/workflows/ci.yml`, 문서 셋.

### 어디까지 했나

**제안까지입니다. 추적이 아닙니다.** 요청은 "자동 얼굴 추적"이었지만 두 단계로 나눴습니다.

1. **(이번) 고정 좌우 중심 제안** — 표본 프레임에서 얼굴을 찾아 `focus_x` 하나를 제안합니다. 적용은 사람이 버튼을 눌러야 합니다.
2. (다음) 시간에 따라 움직이는 자르기 — 프레임마다 crop을 바꾸고 부드럽게 잇는 것.

2번을 먼저 하지 않은 이유는 **검출기가 이 콘텐츠에서 얼마나 맞는지 아직 모르기 때문**입니다. 틀리는 검출기로 화면을 계속 움직이면 안 움직이는 것보다 나쁩니다. 1번을 쓰면서 검출률을 보고 2번을 정하는 편이 낫습니다.

### 규칙

`pipeline/framing.py`에 순수 계산으로 두었습니다. 검출기를 바꿔도 이 규칙은 그대로입니다.

1. 한 장에서 **가장 큰 얼굴** 하나만 씁니다. 뒤에 지나가는 사람까지 세면 가운데로 끌려갑니다.
2. 여러 장의 **가운뎃값**을 씁니다. 평균은 한 장의 오검출에 끌려갑니다.
3. 얼굴이 보인 장이 30% 미만이거나 위치가 화면 폭의 ±25%보다 흔들리면 **제안하지 않고 0.5를 돌려줍니다.** 근거 없는 제안은 사람이 맞춘 값을 망칩니다.

제안에는 항상 근거가 붙습니다(`표본 8/10장에서 얼굴 가운뎃값 70% 지점입니다`). 숫자만 주면 믿을지 말지 알 수 없습니다.
...
`cv2.data` 경로는 있는데 파일이 없었습니다. 그래서 한동안 워커 이미지가 빌드할 때 버전·체크섬을 고정해 받았습니다(`infra/fetch_face_model.py`).

**그리고 파일을 받아 둬도 돌지 않았습니다.** 다음 CI가 다른 곳에서 막혔습니다:

```
이 cv2에는 CascadeClassifier가 없습니다(objdetect 없음). 버전 5.0.0,
자리 /usr/local/lib/python3.11/site-packages/cv2/__init__.py
```

들어온 OpenCV가 **5.0.0**이었습니다. `scenedetect[opencv-headless]`가 버전을 안 박아 두어 새로 나온 5가 들어왔고, 거기에는 `cv2.CascadeClassifier`가 없습니다. XML이 휠에 없던 것도 5.0.0을 재고 한 말이었습니다. 4.x가 싣고 있는지는 아직 재지 않았습니다(임포트 확인이 이제 매번 찍습니다. 싣고 있으면 받아 두는 단계를 지울 수 있습니다).

`pyproject.toml`의 `analysis` 묶음에서 `opencv-python-headless==4.14.0.94`로 못 박았습니다. **5로 올리려면 얼굴 검출을 다른 것(YuNet 등)으로 갈아야 합니다.**

**그리고 받아 두던 단계를 지웠습니다.** XML이 휠에 없다는 말 자체가 5.0.0을 재고 한 말이었습니다. 4.14.0.94 휠을 직접 열어 보니 캐스케이드 XML 17개가 들어 있고 `haarcascade_frontalface_default.xml`도 있습니다. CI도 `cv2.data`에 파일 19개라고 찍습니다. 휠 버전을 못 박았으니 그 안의 XML도 같이 고정됩니다. 빌드할 때 망을 타는 단계 하나와 `infra/fetch_face_model.py`가 사라졌습니다. `R4_FACE_CASCADE`는 다른 캐스케이드(옆얼굴 등)를 쓰고 싶을 때를 위해 남겨 둡니다.

**정면 얼굴만 그럭저럭 찾습니다.** 옆얼굴·가린 얼굴·작은 얼굴은 놓칩니다. 더 나은 검출기(YuNet)는 모델 파일을 받아야 해서 미뤘습니다. 바꿀 자리는 `detect_faces` 하나입니다.

### 무엇을 쟀고 무엇을 못 쟀나

`scripts/verify_faces.py`는 **오검출만** 잽니다. 얼굴이 없는 영상(움직이는 도형, 복잡한 무늬)을 넣고 "못 찾았다"가 나오는지 봅니다. **첫 실측(2026-09-19, OpenCV 4.14.0.94):** 두 영상 모두 표본 7장 중 0장(0%), 제안 `focus_x` 0.5. 무늬를 얼굴로 착각하지는 않습니다. 무늬를 얼굴로 착각하는 검출기는 화면을 엉뚱한 곳으로 옮깁니다. 이 점검은 라벨 없이 매번 돕니다.

**진짜 얼굴을 얼마나 잘 찾는지는 재지 않았습니다.** 얼굴이 든 영상 표본이 저장소에 없고, 사람 얼굴 데이터를 받아 두는 것은 별개의 결정입니다. 그 숫자가 필요하면 실제 영상으로 재야 합니다. **검출률을 모른 채 자동 적용하지 않는 이유이기도 합니다.**

### 검증

- `pytest -q`: **331 통과 / 6 skip**(새 테스트 10개). 전부 검출기 없이 도는 규칙 시험입니다.
- 화면 밖 좌표, 흔들리는 얼굴, 한 장만 보인 얼굴, 얼굴 없음까지 각각 고정했습니다.
- `ruff check`·`ruff format --check`, `npm run typecheck`·`npm run build` 통과.

### 남은 작업

- **검출률 실측.** 얼굴이 든 표본으로 재야 합니다. 그 전에는 제안의 신뢰도를 말할 수 없습니다.
- 시간에 따라 움직이는 자르기(위 2번).
- 세로 중심(`focus_y`)은 건드리지 않았습니다. 얼굴 높이로 정하면 화면이 위아래로 흔들리기 쉬워 따로 봐야 합니다.
...
이 작업을 하다 `media_tasks`의 `kind` 제약을 보고 알았습니다. 앞선 변경에서 API가 `faces`를 받도록 해 놓고 **DB 제약에 넣지 않았습니다.**

```
sqlalchemy.exc.IntegrityError: CHECK constraint failed: ck_media_task_kind
```
...
**시각을 묻지 않습니다. 자막 번호를 묻습니다.**

LLM은 "12분 34초가 좋습니다"라고 그럴듯하게 답하지만 거기에 그 말이 없을 수 있습니다. 지어낸 시각은 확인할 길이 없습니다. 그래서 대본을 `번호<탭>시작<탭>끝<탭>내용`으로 보내고 **번호로 답하게** 합니다. 시각은 우리가 대본에서 꺼내 씁니다. 지어낸 번호는 그 자리에서 걸립니다.

번호를 받아도 그대로 쓰지 않습니다. `pipeline.highlights.accept`가 다시 봅니다.

| 버리는 경우 | 왜 |
|---|---|
| 대본에 없는 번호 | 지어낸 답 |
| 끝이 시작보다 앞 | 뒤집힌 구간 |
| 5초 미만 / 180초 초과 | 숏폼이 아님 |
| 영상 길이를 넘음 | 대본이 영상보다 길게 남아 있을 수 있음 |
| 앞서 고른 것과 겹침 | 같은 장면이 두 번 올라감 |

**버린 것은 이유와 함께 돌려줍니다.** 목록이 짧은 이유가 모델인지 우리인지 알아야 합니다.

### 유료 호출의 자리

`media_tasks`는 "공짜·로컬만", `workflow_tasks`는 "유료 호출 전에 체크포인트"라고 적혀 있습니다. 하이라이트 추천은 더빙 파이프라인의 단계가 아니라 편집하다 누르는 버튼이라 어느 쪽에도 맞지 않아 `worker/highlight_tasks.py`로 따로 뒀습니다. **규칙은 같게 지킵니다: 예약 → 호출 → 정산.**

처음에는 API에서 바로 부르게 썼다가 되돌렸습니다. 두 가지가 틀렸습니다. API 이미지에는 워커 패키지가 없어 **임포트부터 실패**하고, 예산 예약을 건너뛰어 **한도를 넘겨도 아무도 모릅니다.**

단가 `R4_HIGHLIGHT_USD_PER_1K_CHARS`를 적어 두지 않으면 돌지 않습니다. 실제 청구는 토큰 단위인데 글자 수로 잡으므로 **넉넉한 상한**을 적습니다. 번역·더빙 단가와 같은 방식입니다.

### 검증

- `pytest -q`: **368 통과 / 6 skip**(새 시험 28개).
- 공급자는 **대역**으로만 부릅니다. 거절·잘린 답·읽을 수 없는 답·빈 답을 각각 고정했습니다.
- 실패한 호출이 잡아 둔 예약을 **푸는지** 봅니다. HELD로 남으면 예산이 영영 줄어듭니다.
- 단가가 없으면 **공급자가 아예 불리지 않는지** 봅니다.
- 관리화면 `tsc -b --noEmit`·빌드 통과.

### 아직 모르는 것

- **실제로 한 번도 부르지 않았습니다.** 사용자 지시대로 유료 호출은 하지 않았습니다. 추천 품질, 실제 비용, 응답 시간은 재지 않았습니다.
- `anthropic==1.7.0`이 워커 이미지에 실제로 깔리는지는 이 PR의 CI가 확인합니다.
- 한 번에 물어볼 수 있는 대본은 자막 800개·6만 자까지입니다. 넘으면 **자르지 않고 거절**합니다. 긴 영상을 나눠 묻는 것은 하지 않았습니다.
- 추천 순서(모델이 매긴 순)가 실제로 좋은 순인지 모릅니다.

---

## 2026-09-19 — 소음·겹말에서 전사 정확도 실측

- 사용자 요청: "정확도 개선, 강한 소음과 겹말 검수". 브랜치 `claude/claude-md-design-review-v8qdz4`.
- 담당 파일: `packages/pipeline/pipeline/noise.py`(신규), `scripts/verify_robust.py`(신규), `scripts/fetch_korean_speech.py`, `services/worker/worker/{analysis.py,separation.py}`, `.github/workflows/ci.yml`, `tests/test_noise.py`(신규).

### 먼저 쟀습니다

전사 품질은 **깨끗한 낭독 음성**으로만 쟀습니다(CER 9.7%). 소음과 겹말에서 몇 점인지 모른 채 손잡이를 돌리면 나아졌는지 나빠졌는지 알 수 없습니다.

원문을 아는 사람 목소리(zeroth-korean 낭독 3문장)에 분홍 잡음과 **다른 문장을 읽는 다른 목소리**를 정해진 SNR로 섞고 조건마다 CER을 쟀습니다.

### 실측 (2026-09-19 CI, whisper small)

| 조건 | CER | 원음 대비 |
|---|---:|---:|
| 원음 | 10.4% | — |
| 소음 SNR +20dB | 11.2% | +0.7%p |
| 소음 SNR +10dB | 12.7% | +2.2%p |
| 소음 SNR +5dB | 17.2% | +6.7%p |
| 소음 SNR +0dB | 23.1% | +12.7%p |
| 겹말 SNR +10dB | 29.1% | +18.7%p |
| 겹말 SNR +5dB | **92.5%** | +82.1%p |
| 겹말 SNR +0dB | **116.4%** | +106.0%p |

**소음은 생각보다 버팁니다.** 잡음이 목소리와 같은 크기(0dB)여도 23.1%입니다.

**겹말이 진짜 문제입니다.** 끼어든 목소리가 10dB 작아도 29.1%로 뛰고, 5dB부터 무너집니다. 116%는 **원문보다 많은 글자를 뱉었다**는 뜻입니다. 전사가 끼어든 사람의 말을 같이 받아쓰고 있습니다. 실제 출력에 대상 문장과 전혀 관계없는 "박 전 대통령 측은 앞으로 검찰 수사와 관련해…"가 섞여 나옵니다.

### 후보 손잡이는 쓰지 않습니다

`condition_on_previous_text=False`를 같은 음성에 나란히 재 봤습니다.

```
원음 +0.0  소음20 +0.0  소음10 +0.0  소음5 +0.0  소음0 +0.0
겹말10 +0.0  겹말5 -4.5  겹말0 -2.2
```

여섯 조건에서 **차이가 정확히 0**이고, 무너진 두 조건에서만 조금 좋아집니다. 그나마 88%·114%로 여전히 쓸 수 없습니다. **전체가 고르게 좋아진 것이 아니므로 운영 기본값을 바꾸지 않았습니다.** `transcribe(tuning=...)`로 넘길 수 있게만 열어 두었습니다.

차이가 0인 이유는 설명이 됩니다. VAD가 문장 사이 1초 무음에서 끊어 각 문장을 따로 디코딩하므로, 앞 문장을 물고 갈 일이 거의 없습니다. 이 손잡이는 **긴 연속 발화**에서 의미가 있을 값입니다.

### 한계값

조건마다 값이 크게 달라 하나의 상한으로는 못 잽니다. `pipeline.noise.MEASURED_CER`에 실측을 적어 두고 **실측 + 10%p**를 상한으로 씁니다. 비율로 곱하면 무너진 조건에서 무의미해집니다(116%의 두 배는 232%).

### 분리 후 전사도 기각했습니다

목소리만 분리(Demucs)한 뒤 전사해 견줬습니다. **소음에서 크게 나빠집니다.**

```
        원음  소음20  소음10  소음5  소음0  겹말10  겹말5  겹말0
분리 후  +0.0   +0.0   +6.7  +11.2  +20.1   -0.7   -3.7   -5.2  (%p)
```

소음 0dB에서 20.9% → 41.0%로 두 배가 됩니다. Demucs는 음악에서 노래를 떼어내도록 배운 모델이라, 말소리에 깔린 잡음을 걷어내려 하면 말소리까지 깎아 냅니다. 겹말에서는 조금 좋아지지만(−0.7 ~ −5.2%p) 여전히 88~111%라 쓸 수 없습니다. **전사 앞단에 넣지 않습니다.** `--denoise` 손잡이는 손으로 다시 잴 때를 위해 남겨 두고 CI에서는 뺐습니다(오래 걸립니다).

### 재는 방법의 결함 하나

**잡음 씨앗을 고정하지 않았습니다.** 같은 코드로 두 번 돌렸더니 소음 0dB가 23.1% → 20.9%, 5dB가 17.2% → 14.2%로 움직였습니다. 매번 다른 잡음이 깔렸기 때문입니다. 그러면 값이 변했을 때 회귀인지 잡음이 달라진 것인지 구분할 수 없습니다. 씨앗을 박았고(`verify_robust.NOISE_SEED`), **`MEASURED_CER`의 소음 값은 첫 고정 실행으로 갱신해야 합니다.** 겹말 값은 파일이 고정이라 두 번 모두 같았습니다(29.1 / 92.5 / 116.4).

### 아직 모르는 것

- **표본이 낭독 3문장뿐입니다.** 데이터셋 행이 바뀌면 값이 움직입니다.
- 겹말을 실제로 고치는 길(화자별 전사, 겹친 구간 경고)은 손대지 않았습니다. 지금은 **무너진다는 사실을 아는 것**까지입니다.
- 소음은 분홍 잡음 한 종류입니다. 음악·웅성거림은 재지 않았습니다.

---

## 2026-09-19 — 화자별 전사 (겹말에 대한 방향 전환)

- 사용자 요청: "2번부터 해" — 겹말 대응 두 갈래 중 화자별 전사. 브랜치 `claude/claude-md-design-review-v8qdz4`.
- 담당 파일: `packages/pipeline/pipeline/{overlap.py(신규),noise.py}`, `services/worker/worker/analysis.py`, `scripts/{verify_robust.py,fetch_korean_speech.py}`, `.github/workflows/ci.yml`, `tests/{test_overlap.py(신규),test_noise.py}`.

### 왜 방향을 바꾸는가

겹말에서 전사가 무너집니다(실측: 끼어든 목소리가 5dB만 작아도 CER 92%, 같은 크기면 116%). 디코딩 손잡이도(차이 0), 분리 전처리도(오히려 +20%p) 고치지 못했습니다. 섞인 소리를 한 번에 받아쓰는 한 남의 말이 통째로 섞여 나오는 것을 막을 방법이 없습니다.

그래서 **못 알아듣는 구간을 알아듣는 척하지 않기로** 합니다.

### 무엇을 만들었나

1. **겹말 구간 찾기** (`pipeline.overlap.overlap_regions`) — 화자 분리가 준 구간에서 **서로 다른** 화자 둘 이상이 동시에 말한 시간을 찾습니다. 같은 화자가 두 조각으로 나뉜 것은 겹말이 아닙니다.
2. **화자별 전사** (`worker.analysis.transcribe_by_speaker`) — 화자마다 **그 사람 구간 밖을 무음으로 지운 오디오**를 따로 받아씁니다. 겹치지 않은 시간에는 상대 목소리가 아예 들어가지 않으니 남의 말이 섞일 길이 없습니다. 겹친 시간의 자막에는 **겹침 표시**(`SpokenCue.overlap`)가 붙습니다. 무음에 지어낸 자막(그 화자가 말하지 않은 시각)은 버립니다.
3. **부분 겹말 조건** — 지금까지의 겹말 조건은 처음부터 끝까지 100% 겹친 최악의 경우라 이 방법이 도움될 수 없습니다. 실제 겹말은 잠깐입니다. 가운데 1/3만 끼어드는 조건(`partial0`)을 추가했습니다. 진짜 겹친 시각을 우리가 정하므로 겹침 표시의 정밀도·재현율을 잴 수 있습니다.
4. 검증(`verify_robust.py --by-speaker`)은 겹말 조건에서 화자를 나누고(토큰 있으면 pyannote, 없으면 embedding), 우리가 아는 목소리의 자막만 채점합니다: 섞어서 받아쓴 CER → 화자별 CER → 겹침 표시를 뺀 CER. 부분 겹말에서는 표시의 정밀도·재현율도 찍습니다.

**겹친 시간의 자막은 여전히 못 믿습니다.** 이 방법은 겹치지 않은 시간을 살리고 겹친 시간에 표시를 붙이는 것까지입니다. 진짜 겹친 목소리를 가르는 것(target speaker extraction)은 하지 않았습니다.

### 시험을 만들다 잡은 것

`overlap_regions` 시험을 쓰면서 A 0~10, B 4~8, C 7~12의 답을 4~12라고 적었다가 **틀린 것을 잡았습니다.** 10~12는 C 혼자라 겹말이 아닙니다. 답은 4~10이고 코드는 처음부터 맞았습니다. 적어 둡니다.

### 실측 (2026-09-19 CI f154b3a, pyannote로 화자 분리)

화자는 네 조건 모두 둘로 갈렸습니다. 우리가 아는 목소리의 자막만 채점한 값입니다.

| 조건 | 섞어서 | 화자별 | 겹침 뺀 것 | 겹침 표시 |
|---|---:|---:|---:|---|
| 겹말 +10dB | 29.1% | 29.9% | 63.4% | 4개 중 2개 |
| 겹말 +5dB | 92.5% | 88.8% | 100.0% | 6개 중 6개 |
| 겹말 +0dB | 116.4% | 110.4% | 100.0% | 6개 중 6개 |
| 부분 겹말 +0dB | 39.6% | 39.6% | 32.1% | 3개 중 1개 — 정밀도 100%, 재현율 83% |

**화자별 전사는 CER을 내리지 못했습니다.** 네 조건 모두 섞어서 받아쓴 것과 같거나(39.6 = 39.6) 몇 점 안팎입니다(+0.8 / −3.7 / −6.0%p). 이유는 분명합니다. 화자 구간 밖을 지우는 것은 **상대가 혼자 말하는 시간**의 말이 대상 자막에 섞여 들어오는 것을 막는 방법인데, 이 표본에는 그런 시간이 없습니다. 끼어든 목소리는 대상이 말하는 동안에만 말합니다. 대상은 처음부터 끝까지 말하므로 대상 쪽에서 지울 것이 없고, 겹친 시간의 남의 목소리는 그대로 들어갑니다. 실제 대화에서 상대가 혼자 말하는 시간의 자막은 오류가 아니라 **상대의 자막**이고, 그건 화자 분리가 이미 갈라 줍니다. 그래서 화자별 전사가 화자 분리 위에 더해 주는 것이 없습니다.

**겹침 표시는 맞습니다.** 부분 겹말에서 표시한 시간의 100%가 진짜 겹말이었고, 진짜 겹말의 83%에 표시가 붙었습니다. 100% 겹친 5dB·0dB에서는 자막 전부에 표시가 붙었습니다(맞는 판단입니다. 전부 못 믿습니다). '겹침 뺀 것'이 100%인 것은 표시된 자막을 다 버려 아무것도 남지 않았다는 뜻입니다. 겹말 10dB에서는 4개 중 2개에만 붙었습니다. 끼어든 목소리가 10dB 작으면 화자 분리가 그 사람을 절반의 시간에서만 듣습니다. **작게 끼어든 말은 표시가 놓칩니다.** 표시된 자막을 버리면 63.4%로 오르는데, 대상의 말도 같이 버렸기 때문입니다. 표시는 **믿을 수 없다는 알림**이지 지우라는 뜻이 아닙니다.

**운영 경로에 넣지 않습니다.** 나아지지 않은 것을 넣을 이유가 없습니다. `transcribe_by_speaker`는 검증 도구로 남깁니다. 이 실측이 뒷받침하는 다음 후보는 화자별 전사가 아니라 **겹침 표시만** 운영에 넣는 것입니다(처음 두 갈래 중 1번). 화자 분리 결과에서 `overlap_regions`를 구해 그 시간에 걸친 자막에 표시를 붙이면 되고, 전사는 지금처럼 한 번 합니다. 자막 편집 화면에서 "여기는 두 사람이 겹쳐 말해 못 믿는다"를 보여 주는 것까지입니다. 넣을지는 사용자가 정합니다. 이 PR에서는 하지 않았습니다.

---

## 2026-09-19 — 전사기 자체가 흔들립니다 (반복 측정을 넣음)

- 담당 파일: `scripts/verify_robust.py`, `packages/pipeline/pipeline/noise.py`(주석), `tests/test_noise.py`, `.github/workflows/ci.yml`.

### 무엇을 봤나

잡음 씨앗을 고정한 첫 실행(71403b1)의 값이 고정 전과 **또 달랐습니다.** 그런데 이번에는 잡음 탓이 아닙니다.
...
겹말 파일은 처음부터 고정이었는데 0dB가 116.4% → 98.5%로 18%p 움직였습니다. **전사기가 같은 소리에 매번 다르게 답합니다.** 소음 조건은 ±3%p, 무너진 겹말은 ±18%p입니다. 소음 조건끼리 순서가 뒤집히는 것(20dB 13.4 > 10dB 10.4)도 이 폭 안입니다.

원인으로 의심되는 곳은 faster-whisper의 temperature fallback입니다. 첫 시도가 미덥지 않으면 온도를 올려 다시 뽑는데, 그때 표본을 무작위로 뽑습니다. 무너진 조건일수록 이 경로를 자주 타니 폭이 커지는 것과 맞습니다. **확인은 안 됐습니다.** 그래서 잽니다.

### 무엇을 바꿨나

- `--repeat N`: 조건마다 N번 받아써 표에 **가장 나쁜 값과 폭(최대-최소)**을 남깁니다. 상한 비교는 가장 나쁜 값으로 합니다. 한 번 잰 값으로 상한을 적으면 다음 실행이 우연히 넘습니다.
- `--seed-decoder`: 디코더 난수를 고정한(`ctranslate2.set_random_seed`) 설정을 나란히 잽니다. 이 설정의 폭이 0이면 원인이 난수이고, 아니면 다른 데(스레드 순서 등)입니다.
- CI는 `--repeat 3 --seed-decoder --by-speaker`로 돌립니다. 기각한 `--compare`는 뺐습니다(여섯 조건 차이 0).
- `test_the_limits_get_harder…`는 소음 조건끼리의 순서를 더 묻지 않습니다(원음 ≤ 소음 ≤ 겹말, 겹말끼리는 순서). 폭 안에서 자리가 바뀌는 것을 회귀로 잡으면 안 됩니다.

### 앞 절의 정정

앞에서 "`MEASURED_CER`의 소음 값은 첫 고정 실행으로 갱신해야 한다"고 적었습니다. **틀렸습니다.** 한 번 잰 값은 여전히 한 번 잰 값입니다. 반복 측정의 가장 나쁜 값으로 갱신합니다. 표는 아직 고정 전 값 그대로입니다.

### 실측 (f154b3a): 폭은 0, 그런데 실행 사이에는 다릅니다

**한 실행 안에서는 폭이 0입니다.** 아홉 조건 모두 세 번이 글자 하나까지 같았고, 씨앗 고정도 아홉 조건 모두 차이 0입니다. **난수가 아닙니다.** 위에 적은 temperature fallback 의심은 틀렸습니다. 그런데 실행 사이에는 여전히 다릅니다.
...
같은 코드, 같은 씨앗, 같은 파일인데 실행마다 다르고, 한 실행 안에서는 똑같습니다. 남는 설명은 **기계가 다르다**는 것입니다. CTranslate2의 int8 CPU 커널은 CPU가 지원하는 명령어 집합(AVX2/AVX512)과 제조사에 따라 다른 경로를 타고, GitHub 러너는 실행마다 다른 CPU를 줍니다. 부동소수점이 조금 다르면 갈림길에 있던 토큰이 바뀌고, 무너진 조건일수록 갈림길이 많아 폭이 큽니다(겹말 0dB 18%p).

**확인은 안 됐습니다.** 그래서 두 가지를 넣었습니다. (1) 로그 첫 줄에 CPU 이름과 avx2/avx512f 플래그를 찍습니다. 값이 달라진 실행의 CPU가 다르면 설명이 맞는 것입니다. (2) `--cpu-isa AVX2`로 CTranslate2가 쓸 명령어 집합을 고정합니다(`CT2_FORCE_CPU_ISA`). 다음 실행이 다른 CPU에서 위 f154b3a 값과 같게 나오면 고쳐진 것입니다. 제조사별 커널 차이(MKL/oneDNN)까지 잡는지는 모릅니다.

`--repeat`과 `--seed-decoder`는 손으로 잴 때를 위해 남기고 **CI에서 뺐습니다.** 한 실행 안에서 폭이 0이라 CI 시간만 씁니다(세 번 받아쓴 이번 실행의 이 단계가 12분).

`MEASURED_CER`은 씨앗을 고정한 두 실행의 **큰 쪽**으로 갱신했고 `partial0`(39.6%)를 넣었습니다. 상한은 거기에 10%p입니다. 겹말 0dB는 실행 사이 폭이 18%p라 여유가 모자랄 수 있습니다. 넘으면 먼저 CPU 이름을 지난 실행과 견주고, 회귀가 아니면 여유가 아니라 실측을 고칩니다.

### 실측 (0bb9e57): CPU를 찍은 첫 실행

`CPU AMD EPYC 7763 64-Core Processor (avx2), 명령어 집합 고정 AVX2`. 아홉 조건 모두 f154b3a와 **글자 하나까지 같습니다**(10.4 / 12.7 11.2 14.2 25.4 / 29.1 92.5 116.4 / 39.6). 화자별 전사 값도 같습니다.

**이것으로 CPU 설명이 맞다고는 못 합니다.** 이 CPU에는 AVX512가 없어 AVX2 고정이 실제로 바꾼 것이 없고, f154b3a와 71403b1이 어떤 CPU였는지는 그때 찍지 않아 모릅니다. 두 실행이 우연히 같은 종류의 러너였을 수 있습니다. 설명이 맞으려면 **다른 CPU**(Intel, AVX512)를 받은 실행이 같은 값을 내야 합니다. 그때까지는 미확인입니다. 반복을 뺀 이 단계는 4.5분입니다(12분 → 4.5분).

---

## 2026-09-22 — 겹침 표시를 운영에 넣음

- 사용자 요청: "겹침 표시 넣어줘". 브랜치 `claude/claude-md-design-review-v8qdz4`(최신 main에서 새로 시작).

### 왜

2026-09-19 실측에서 겹말은 전사로 고칠 수 없다는 것까지 확인했습니다(CER 92~116%, 디코딩 손잡이 차이 0, 분리 전처리 +20%p, 화자별 전사도 내리지 못함). 그때 남긴 다음 후보가 **겹침 표시만 운영에 넣는 것**이었고, 계산(`pipeline.overlap`)과 정확도(부분 겹말에서 정밀도 100%·재현율 83%)는 이미 있었습니다. 남은 것은 화면까지 잇는 일이었습니다.

### 무엇을 했나

1. `transcript_segments.overlap`(NULL 허용 boolean) 추가 — 마이그레이션 `0011_transcript_overlap`.
2. 화자 분리 작업이 새 대본 버전을 만들 때 `overlap_regions(turns)` → `flag_overlaps(cues, regions)`를 그 자리에서 찍습니다. 전사를 다시 하지 않고 이미 가진 화자 구간만 씁니다. 결과에 `overlap_regions`(구간 수), `overlap_seconds`, `overlapped`(표시된 자막 수)를 남깁니다.
3. `GET /source-assets/{id}/transcript`가 `speaker`·`overlap`을 함께 줍니다. 저장 요청은 이 두 값을 **받아서 버립니다**(`extra="ignore"`) — 읽은 것을 그대로 돌려보내도 저장이 되게 하려고요.
4. `GET /source-assets/{id}/speakers`에 `overlap: {checked, count, seconds, cue_numbers}`. `checked=false`는 **안 재 봤다**는 뜻입니다.
5. 편집 화면: 「화자 분리·겹침 표시」 버튼, 겹침 요약 알림, 걸린 자막 줄마다 표시.

### 잡은 것: 화자 분리를 부를 화면이 없었습니다

API(`POST /source-assets/{id}/diarize`)와 워커는 2026-09-18부터 있었지만 **편집 화면에 버튼이 없었습니다.** 사람이 부를 길이 없으니 겹침 표시를 붙여도 아무도 보지 못합니다. 버튼을 넣었습니다. `worker.highlight_tasks`가 celery `imports`에 빠져 있던 것과 같은 종류의 구멍입니다 — 코드는 있고 사람이 닿는 길이 없는 것.

### NULL을 남겨 둔 이유

`overlap`을 기본 `false`로 두면 예전 행과 화자 분리를 돌리지 않은 대본이 "겹침 0건"으로 읽힙니다. **재지 않은 것과 없는 것은 다릅니다.** NULL로 두고 `/speakers`의 `checked`로 구분합니다. 사람이 대본을 고쳐 만든 새 버전도 NULL입니다 — 글자를 고치면 시각이 바뀔 수 있어 옛 측정을 새 시각에 붙일 근거가 없습니다.

### 검증

- `pytest -q`: 597 통과, 3 건너뜀(모델 내려받는 검증). 새 시험 6개: 겹말 구간에 걸친 자막에 표시가 붙고 걸치지 않은 자막에는 안 붙는지, 번갈아 말하면 표시가 없는지, 예전 버전은 NULL로 남는지, 읽은 응답을 그대로 저장할 수 있는지, 요약이 맞는지, 안 재 본 대본이 "겹침 0"이 아니라 "안 재 봄"으로 나오는지.
- `ruff check .` / `ruff format --check .` 통과. `alembic upgrade head → downgrade base → upgrade head` 통과 후 열 확인.
- `npx tsc -b --noEmit`, `npm run build` 통과.

### 남은 문제

- 실제 여러 사람이 말하는 영상으로 화면까지 돌려 보지 않았습니다(pyannote 토큰 필요). 유료 호출·업로드는 하지 않았습니다.
- 작게 끼어든 말(10dB)은 화자 분리가 절반만 들어 표시가 놓칩니다. **표시가 없다고 겹치지 않은 것은 아닙니다.**
- 판정 기준 `MIN_OVERLAP_FRACTION=0.2`은 2026-09-19에 정한 값 그대로입니다. 실제 영상에서 표시가 너무 많거나 적으면 다시 잴 값입니다.

### 곁들여 고친 것: 표본 없이 도는 pyannote 검증 단계

PR #39의 CI에서 `화자 분리 검증 (pyannote)`가 `FileNotFoundError: /audio/expected.json`으로 실패했습니다(run 35680350814). 이 PR의 변경과 무관합니다. 표본을 만드는 앞 단계는 `verify-align` 라벨에만 걸려 있고 그 표본을 쓰는 이 단계는 `env.HF_TOKEN != ''` 하나로만 걸려 있어서, **저장소에 HF_TOKEN이 등록된 뒤로는 라벨 없는 모든 PR이 여기서 실패합니다.**

같은 조건을 PR #20과 #36이 이미 갖고 있어 그 한 줄을 옮겼습니다(라벨/커밋 표시 + 토큰). 같은 CI 파일의 다른 검증 단계 전부가 이미 그 형태입니다. 토큰이 없을 때 도는 「화자 분리 토큰 오류 안내 점검」과 짝이 맞습니다.

교훈은 **표본을 만드는 단계와 쓰는 단계의 조건은 같아야 한다**는 것입니다. 다르면 조건이 느슨한 쪽이 빈 디렉터리를 보고 엉뚱한 자리에서 터집니다.

...
- `compare_conversation_evaluation.py` 추가: 실제 manifest/음원/정답을 재검사한 lock과 양쪽 점수 파일을 연결합니다. 해시·언어·분모·전체 조건 범위를 확인하고 기존 정답 증가/오배정 비증가 규칙을 적용합니다. 실패/거부도 분모에 남기고 개선 판정을 거부합니다. 언어별 수치를 출력하고 기존 결과 덮어쓰기를 차단합니다.
- 실제 CLI 회귀에서 정상 비교 후 음원을 변경하면 실패 코드 1, 기존 결과 보존 확인. 새 13개 테스트, Python 전체 **535통과/5skip**, Ruff 통과. 테스트 데이터는 합성 fixture이며 실제 새 음성 정확도 측정은 아닙니다.

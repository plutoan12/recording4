...
- 사람별 발화 정답이 없어 정확도 개선은 아직 미입증이다. 재현 집계와 보정 게이트 회귀를 추가했고 전체 Python **689통과/20skip**, Ruff 검사·형식 검사가 통과했다. 자동 재배정·운영 기본은 그대로고 PR37 초안을 유지한다. [실측과 한계](quality/active-speaker-youtube-2026-09-20/REPORT.md).
...
- 실제 영상 출력은 확보했지만 사람별 발화 정답과 점수 보정이 없어 정확도 개선 실측은 없다. 관련 검사와 전체 Python **689통과/20skip**, 전체 Ruff 검사·형식 검사가 통과했다. CI는 새 HEAD에서 확인한다. [계약과 한계](quality/target-independent-evidence-2026-09-20/REPORT.md). PR37 초안과 운영 기본을 유지한다.

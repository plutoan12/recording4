...
- 관련 회귀 15개(편집 옵션·모자이크 명령·실제 FFmpeg 렌더)가 워커 이미지에서 통과했다. 현재 브랜치의 기존 전체 CI는 직전 `b3bfab4`에서 성공했고 이 변경은 새 CI를 기다린다.
- 지정 영상의 후보 15개에 앞뒤 1초를 붙인 16 kHz mono WAV와 `labels.template.json`을 `/Users/an-youwon/Projects/recording4-evaluation-private/active-speaker-youtube-20260920/review-pack-20260921`에 만들었다. 모든 `human_*` 값은 비어 있다. 저는 이 환경에서 음성을 청취할 수 없으므로 사람 화자 정답을 임의로 작성하지 않았다.
- 다음 단계는 사람이 각 WAV를 듣고 `speaker_0`, `speaker_1`, `multiple`, `unknown`을 기록한 뒤, 같은 분모로 DER/CER·오배정·누락을 다시 계산하는 것이다. 그 전까지 운영 자동 재배정과 모델 교체는 하지 않는다.
...
- Light-ASD 공식 출력은 0~1 확률이 아닌 원시 로그잇이며 추적별 마지막 1프레임, 고유 8프레임·0.32초가 미채점이다. 기계 시각 근거는 보정 확률 선언·revision과 별도 신뢰 목록의 조합 없이는 대상 화자 자격 판정에 들어갈 수 없다. 현재 신뢰 목록은 비어 있다. 원본·음원·얼굴·원시 점수는 비공개 폴더에만 보존했다.
- 사람별 발화 정답이 없어 정확도 개선은 아직 미입증이다. 재현 집계·검수 구간과 보정 게이트 회귀 3개를 추가했다. 직전 전체 Python **689통과/20skip** 이후 변경분은 새 CI에서 확인한다. 자동 재배정·운영 기본은 그대로고 PR37 초안을 유지한다. [실측과 한계](quality/active-speaker-youtube-2026-09-20/REPORT.md).

## 2026-09-20 대상 화자 ASR 독립 근거 연결

- 대상 화자 ASR의 단일 문구 후보에 독립 음성 신원과 활성 화자 시각 근거를 같은 단어·시각·음원 SHA로 연결하는 검증 계약을 추가했다. 기존 화자·글자·시각은 바꾸지 않고 자격 여부와 거부 사유만 기록한다.
- 음성 기준은 기존 0.6 유사도/0.15 경쟁 margin을 유지한다. 시각 근거는 활성 화자 탐지와 사람 영상 검수를 구분하며 0~1 점수와 개발 후보 문턱 0.7을 검사한다. 기계 시각 점수는 보정 확률 선언·revision과 별도 신뢰 조합을 필수로 요구한다. 다른 대상 근거, 복수 대사 후보, 오래된 음원, 다른 단어, 비유한/범위 밖 점수는 거부한다.
- `automate_speaker_review.py`가 선택적인 비공개 근거 파일을 받아 조건별 `independently_qualified_words`를 집계한다. 검사를 실행하지 않은 `absent`와 실행한 `evaluated`를 구분하며, null/객체 입력과 제안에 연결되지 않은 근거는 거부한다. 모델 revision 또는 사람 검수자/시각을 감사 기록에 보존하고, 두 사람 판정은 서로 다른 검수자를 요구한다. 재검사에서 과거 출처가 남지 않는다. 통과해도 `applied=false`, `changed_assignments=0`, `deploy_allowed=false`다.
- 실제 영상 출력은 확보했지만 사람별 발화 정답과 점수 보정이 없어 정확도 개선 실측은 없다. 관련 검사와 직전 전체 Python **689통과/20skip**은 통과했고 이번 3개 회귀와 전체 Ruff는 새 CI에서 확인한다. [계약과 한계](quality/target-independent-evidence-2026-09-20/REPORT.md). PR37 초안과 운영 기본을 유지한다.

## 2026-09-20 공백 포함 CJK 단어 경계 복구

- 실제 한·일·중·영 대화 각 4구간을 `stable-ts 2.19.1`/`faster-whisper-small`로 다시 정렬했다. 입력 SHA `ccc656a8`, 정렬 결과 SHA `1c8e7c4c`; 원음·대사는 비공개 평가 폴더에만 보존한다.
- 기존 공백 경계 규칙은 중국어·일본어 정렬기가 한자·가나를 공백보다 작은 조각으로 반환하면 거의 모두 버렸다. 사람 대본 문자 중 유효 시각이 중국어 **2/110→99/110**, 일본어 **2/110→105/110**으로 회복됐다. 한국어는 **63/67**, 영어는 **100/107**로 변화가 없었다.
- 사람 화자 시각으로 경계 로직만 격리해 확인했을 때 중국어는 단일 화자 89자·실제 겹말 10자·미확인 11자·오배정 0자, 일본어는 단일 화자 105자·미확인 5자·오배정 0자였다. 반복 CJK 토큰은 전체 토큰열의 최대 문자 정렬에서 위치가 하나로 확정될 때만 쓴다.
- `review_speakers`는 원래 공백과 한자·히라가나·가타카나·한글에 닿는 위치를 유효 경계로 본다. 라틴/숫자 단어 내부의 하이픈·소수점은 경계로 만들지 않는다. 공백 포함 영어 `one`/`someone` 방어와 공백 없는 비CJK 언어의 기존 순차 정렬도 검사로 보존했다. 이는 정렬 문자 보존 개선이며 실제 모델 DER/CER나 네 언어 전체 정확도 개선을 입증한 결과는 아니다. [상세](quality/cjk-boundary-2026-09-20/REPORT.md).
- 반복 문자 방어와 4만 정렬 상태 상한을 넣은 최종 matcher SHA `3b2cdb24`로 16구간을 다시 실행해 같은 수치를 확인했다. 비교 파일 SHA는 `d63c826d`; 결과 JSON에 둘 다 연결했다. 토큰별 후보 위치를 한 번만 계산하도록 바꿔 200자 반복 CJK 대역의 경계 계산은 이 Mac에서 34.6ms였다. 상한을 넘는 비정상 장문 자막은 화자를 추측하지 않고 검수 대상으로 남긴다.
- 검증: 기본 의존성 전체 Python **646통과/20skip**, Ruff 검사·형식 검사 통과. 선택 의존성 및 워커/스택은 새 HEAD의 CI에서 확인한다.
...
- 고정 중국어/일본어/영어 각120초에서 단독 FireRed는 정밀도가 높지만 누락이 컸다. miss/false alarm은 중국6.82/2.11초, 일본6.329/2.876초, 영어30.61/0.27초. Silero는0/5.406,3.857/14.258,7.054/2.006초다.
- Community+FireRed 합집합 총 음성 오류는 일본8.884→8.839초, 영어9.390→9.337초로 아주 조금 감소했으나 중국2.612→3.392초로 악화했다. 표본당 한 대화이고 한국어가 없어 언어별 선택·운영 연결을 하지 않았다. [실측](quality/firered-vad-2026-09-20/REPORT.md).
- 고정 음원/사람 시각/모델 revision에 묶인 음성 존재 miss·false alarm 채점기와 9개 회귀를 추가했다. 외부 모델·음원·확률은 커밋하지 않았다. 정확도 개선·네 언어 개선 미입증, PR37초안 유지.
- 전체 Python 회귀는 680통과/2skip, Ruff 검사·형식 검사 통과다.

## 2026-09-20 독립 영어 경계 후보·확장 기준 검증

- 기존 평가와 PCM이 다른 AMI test row2 첫120초를 고정했다. Community16발화/2화자, Sortformer34발화/3화자, Whisper78단어. 같은 고정 규칙으로 공통 빈 구간2개/1.6초와 DiCoW 부분 합의 후보1그룹/7단어를 생성했다. 사람 정답은 선택·추론·승인에 쓰지 않았다.
- 같은 회의300초 Community29발화로 대상 화자만 공유구간에서 대응했다. coverage98.02%/purity99.67%/유일 양방향 최적으로 통과, 이후 대상65.81초·경쟁79.14초 기준 확보. 그러나 ECAPA0.2392<0.6, margin0.1235<0.15, 독립Whisper불일치로 채택0. [실측](quality/independent-boundary-2026-09-20/REPORT.md).
- 전체 원래 화자 대응을 강제해 대상까지 쓰지 못하던 검사를 후보 대상 대응으로 세분화했다. 확장 근거가 대체하는 과거 음성 사유만 제거하고 독립ASR·짧은후보·알수없는사유는 유지한다. Community 오프라인 실행 CLI와 회귀를 추가했다.
- 전체 Python 회귀는 670통과/2skip이고 Ruff 검사·형식 검사도 통과했다.
- 후보 선정이 끝난 뒤 AMI 공식 화자 시각으로 사후 대조하니 47.46~48.34초 후보는 실제 발화가 없는 구간이었다. Whisper 무음확률/로그확률은 오검출0.392/-0.987, 짧은 실제 발화0.446/-1.017로 역전돼 단순 신뢰도 문턱은 실제 발화도 버린다. 이 문턱은 채택하지 않았다.
- 앞선 두 파일은 WAV SHA만 달랐고 PCM이 기존과 같아 독립 표본에서 제외했다. 원시는 비공개 independent-boundary-dev*-20260920에만 저장. 운영/유료/게시 변경 없음, 정확도 개선·네언어 개선 미입증, PR37초안 유지.

## 2026-09-20 경계 불일치 부분 합의 후속

- DiCoW 두 정렬기 중 한 경계 단어만 핵심 포함 판단이 다를 때, 같은 행의 독립 합의 단어까지 전부 버리던 후보 손실을 수정했다. 불일치 단어는 제외하고 선택된 단어·시각을 별도 감사 필드로 넘긴다. 저장 정렬의 입력 SHA를 재검증하는 재선택 CLI와 회귀 검사를 추가했다.
- 실제 영어 고정 3구간/6출력 재선택에서 부분 합의 1행·2단어 후보가 생겼다. 독립 ECAPA 유사도0.1927<0.6, 경쟁 기준 부재, 별도 Whisper 불일치로 채택0. 기존 자막/784자 분모/운영 기본은 그대로이며 정확도 개선 미입증. [실측](quality/extended-anchor-2026-09-20/REPORT.md).
- 격리 재정렬은 NLTK punkt_tab 부재로 출력 전 실패해 기존 정렬 시각을 해시 검증해 재사용했다. 독립 검증은 오프라인 환경을 코드에서 강제해 로컬 모델 부재 시 외부 재시도를 막았다. 원시는 비공개 consensus-gap-dicow-20260920에만 저장, 유료/게시 변경 없음.
- 워커 환경 전체 Python **663통과/2skip**, 관련15개·전체 Ruff 린트/포맷·공개 JSON·diff 검사 통과. 오래된 개발 이미지는 NumPy 부재로 수집 전에 중단돼 최종 검증에는 사용하지 않았다.
- 다음은 독립 개발 대화에서 경쟁 화자 기준이 있는 누락 후보 검증 또는 한국어 사람 정답 완성. 새 네 언어 전체 개선은 미입증, PR37초안/자동화PAUSED 유지.

## 2026-09-20 동일 회의 300초 기준 음성·분리 재인식

- 기존 영어 평가와 샘플 단위로 같은 300초 원본을 Community/Sortformer로 재분리. 첫120초 시간 대응은 Community 두 화자 모두 양방향99.95% 이상으로 통과. 이후 대상 단독149.592초를 확보했지만 후보 유사도0.1566<0.6, 다른 화자 단독0초였다.
- Sortformer 이후 단독 기준105.03초/1.76초와 후보 유사도0.1541/0.0549. 익명ID 대응 증명 없이 진단에만 사용. SepFormer 두 채널 별도ASR도 정확 일치0/2, 대사 유사도0.1429/0. [실측·한계](quality/extended-anchor-2026-09-20/REPORT.md).
- 두 화자 모델 공통 음성에서 ASR 단어±0.1초를 뺀0.5초 이상 빈 구간3개/1.963초를 정답 없이 선정. 문맥 재인식 새 후보0개. 추가 후보2단어 채택0, 기존 출력/784자 분모 유지, 정확도 개선 미입증.
- 같은3구간×Community2화자 DiCoW6회 정상/잘림0. 비어있지 않은 출력4개→두 정렬기 동의1개→핵심 단어0개로 추가 후보0. 대본 입력 없음, 문맥 대사를 핵심에 넣지 않음. [전체 실측](quality/extended-anchor-2026-09-20/REPORT.md).
- 선정 후 사람 시각 사후 대조:3구간이 사람 단어4개/13자와 겹치지만 중심점 포함은1개. 실제 말 경계 진단이며 후보선정/문턱변경에 사용하지 않음.
- Python633통과/7skip, 관련18개·전체 Ruff포맷·변경 린트 통과. 이전 c32149e CI Python 실패는 두 스크립트 포맷 누락이며 이번 변경에서 수정. 원시는 비공개 consensus-gap-dicow-20260920, 유료/게시/운영 변경 없음. 다음은 짧은 경계 단어의 독립 정렬 근거 또는 한국어 사람 정답 완성. 네 언어 전체 검증 미완료, PR37초안/자동화PAUSED 유지.

## 2026-09-20 기준 음성 0초 원인 분리

- Community 두 번째 화자의 예측 1.85625초 전부가 겹말이라 단독 기준 0초. Sortformer 대체 후보는 단독1.68초 중 짧은 조각1.04초 제외 후0.64초로 총1초 기준 미달이다. 원인은 서로 다르며 문턱을 완화하지 않았다.
- anchor_availability로 예측 합집합/겹말/짧은 조각/후보 문맥 손실을 나눠 실제 검증에 연결했다. 실제 두 모델의 변경 전후 선택 구간 동일. [집계와 다음 실험](quality/anchor-loss-2026-09-20/REPORT.md).
- 전체622통과/7skip, 마지막 계측 보완 후 관련7개 통과, Ruff통과. 새 모델 추론 없이 저장 예측 진단, 정확도 개선 주장 없음. 시작 aa8ffaf Python/웹/DER CI통과, 워커/스택은 확인 당시 진행 중.
- 다음은 더 긴 동일 녹음 또는 독립 개발 표본에서 기준 음성 확보·익명ID 대응 검증. 기존 후보의 낮은 유사도/별도 ASR불일치도 미해결. 평가구간·분모 유지, PR37초안/운영기본/자동화PAUSED 유지.

## 2026-09-20 기존 대사 보존·누락 추가의 실제 음성 검증

- 별도 실험 CLI로 기존 단어/시각/순서를 모두 보존하고 두 정렬기 동의 결과의 빈 구간만 후보로 추출했다. 실제 영어 후보 1묶음/2단어, 채택 0단어. 대상 유사도 0.1413<0.6, 경쟁 기준 음성 0초, 별도 ASR 대사 불일치로 거부했다.
- 전체 784자 기준 기존/검증 후 CER 21.8112%(171오류)로 동일. 검증 없이 추가한 사후 비교는166오류이나 미적용이며 치환1증가. 정답을 보고 채택하지 않았다. [실측·방법·한계](quality/additive-retry-2026-09-20/REPORT.md).
- Python620통과/7skip, Ruff통과. 최종 준비 결과 동일 재현/기존 출력 보존 확인. 비공개 additive-retry-20260920에 원시 저장, 공개 집계만 커밋. 네트워크 차단 무료 로컬 추론, 운영 변경/유료 호출 없음.
- 다음은 경쟁 기준 음성 부재와 낮은 대상 유사도/짧은 대사 재인식 실패의 독립 개발 자료 진단. 네 언어 개선은 미입증, 한국어 사람 정답 및 새 네 언어 최종 평가 미완료. PR37 초안/자동화 PAUSED 유지.

## 2026-09-20 긴 문맥+두 정렬기 실제 검증

- 같은6핵심구간/115자/48평가칸을유지하고 앞뒤3초추가DiCoW30개정상종료. 생성대사만stable-ts/WhisperX에정렬해텍스트보존/핵심포함/0.5초시각동의검사. 동의12/빈생성5/불일치등기존결과유지13.
- Community오류75→77/일치56→44, Sortformer오류106→97/일치25→18. 삽입은줄었지만삭제가늘어대사복구성공으로보지않는다. 자동화자/자막변경0,운영채택보류. [실측/한계](quality/context-dicow-2026-09-20/REPORT.md).
- 모듈경로오류수정과NLTK punkt_tab공식자료보완후네트워크차단별도컨테이너에서정렬30개완료. DiCoW는재추론하지않음. Python615통과/7skip,Ruff통과,준비manifest동일재현. 원시는비공개context-dicow-20260920.
- 다음은기존맞는대사보존+누락후보추가와독립음성근거검증.2단계음성기준/새네언어사람정답/전체정확도입증은여전히미완료. PR37초안/운영기본/자동화PAUSED유지.

## 2026-09-20 4단계: 실제 영어 국소 DiCoW30개 실행 완료

- 고정 영어120초에서 기존예측만으로 겹말불일치6구간 선정(문맥0.5초). Community2화자/Sortformer3화자를 모두 CPUfloat32로 디코딩했다.30/30정상종료, 길이제한의심0. 정답으로마스크/구간을수정하지않았다.
-4정답화자×6구간×2방법=48평가칸, 같은115자 분모. Community입력75오류(65.22%), Sortformer입력106오류(92.17%). 빈정답화자의삽입4/12, 누락화자정답9/9. 잘린단어11개를포함하는 보수적국소CER이며 기존Whisper/전체영상/이전통제표본 점수와직접비교금지.
- 자막자동합치기/화자변경0. 문맥부족/경계절단/decoder attention-mask경고가 한계이며 단어시각없는출력을 안전하게합치는방법은 미완료. [방법/실측/재현](quality/local-dicow-2026-09-20/REPORT.md).
- prepare_local_dicow로 실행manifest/slots데이터동일재현, scorer는누락화자와빈정답출력도보존한다. Python611통과/7skip, Ruff통과. 원시는 비공개local-dicow-20260920만 사용, 유료/운영변경없음. PR37초안/자동화PAUSED유지.
- 다음은예측발화경계의 더긴문맥과출력시각연결 검증.2단계음성근거미달/3단계ASR누락선정범위/5단계한국어사람정답과새4언어전체검증은 여전히미완료다.

## 2026-09-20 3단계: 국소 재인식 실제 영어 실험

- 두 화자 모델이 발화 유무를 다르게 예측한 구간을 정답 없이 선택해 문맥1초를 더한 국소 Whisper 재인식/중심시각 기준 단어 교체를 구현했다. VAD는 기준on/크롭off, 화자배정변경 없음.
- 실제120초: 핵심0.335781초/11크롭, CER21.8112→21.6837%(171→170오류/784자). 삭제158→153이나 치환10→14, 삽입3그대로. 매우작은 변화/치환증가로 채택보류. 기존801자 화자oracle와 다른 정규화 분모다.
- 기준단어 시각1개 불확실은 원문 보존. 초기2실행의 시각처리 오류 수정 후 성공1실행, 크롭실패0. 실제추론/원시 결과는 비공개 local-retry-20260920. 정답/원음/핵심구간 선정방법변경 없음.
- 전체608통과/7skip+추가 정규화회귀1개, 신규6검사/Ruff통과. [방법/수치/한계](quality/local-retry-2026-09-20/REPORT.md). ASR누락과 화자모델누락은 달라 이번선정으로 큰누락 대부분을 못잡았다. 다음4단계 국소DiCoW 및 ASR빈구간/음성활동 연결,2단계음성근거 실패/5단계새네언어는미완료. PR37초안/운영기본/자동화PAUSED유지.

## 2026-09-20 2단계 후속: 대체 기준 목소리 실제 음성 검사

- Sortformer의 예측에서50% 겹말 대상 후보를각3.84초 찾고 ECAPA로 서로 겹치지 않는 절반의 목소리 일관성/다른 화자와의 차이를 실제 측정했다. 정답/전사를 모델 입력으로 사용하지 않았다.
- 기존11조건 중9조건은 두 화자 후보 기준을 만족하지만50% 두 조건의 짧은 후보는0.5911/0.4729로 기존0.6기준 미달이다. 차이는0.4231/0.3998로0.15이상. 문턱을 낮추지 않았고 통과도 실제화자ID/음성순도 승인은 아니다.
- 독립 컨테이너/네트워크차단/읽기전용 캐시,11조건 새임베딩 추론 완료. 후보만검사한 것으로 정답 회복0자/자동배정변경0. [실측](quality/anchor-probe-2026-09-20/REPORT.md).
- Python603통과/7skip, Ruff통과. 원시음원/예측은 비공개 anchor-probe-20260920에 보존. 다음은3단계국소누락/짧은맞장구 재인식→4단계국소DiCoW,2단계실패의 독립음성·ID검증 및5단계한국어사람정답/새네언어도 미해결. PR37초안/운영기본/자동화PAUSED 유지.

## 2026-09-20 순서대로 개선 요청: 1단계 후보 평가와 2단계 원인 확인

- 최소 단어 화자 시간 지지 옵션을 실험으로 구현했다. 기본0으로 운영 동작 유지.80% 후보는 중복 합집합으로 계산하고 근거 부족 배정은 미확인으로 보존한다.
- 실제 영어 Community695/32/74→646/21/134, Sortformer715/32/54→528/32/241. 기존11조건3767/109/513→3700/109/580. 정답도 잃으므로 후보 채택하지 않았다. 원본SHA/재생성 일치 확인 후 저장 예측 재채점이며 모델추론 없음.
- 2단계 기준 목소리0초 원인을11조건 캐시로 조사했다.50% 겹말 두 조건의 한 화자는 조각을 합쳐도0초로 유지. 재정렬·독립 음성 근거로 미확인을 회복하는 후속은 아직 미완료다.
- Python599통과/7skip, DER23통과, Ruff통과. [수치와1~5진행 상태](quality/word-coverage-2026-09-20/REPORT.md). 다음은2단계 독립 음성 검증 후보,3단계 국소 누락 복구,4단계 국소DiCoW,5단계 사람 정답·새4언어 검증 순서다. 전체완료/정확도상승으로 표현하지 않는다. PR37초안/자동화PAUSED/운영기본 유지.

## 2026-09-20 네 언어 전체 개선 입증 요청: 현재 후보 불합격 확인

- 고정 저장 실측에서 모델교체/음량보정의 언어별9개 비교를 재계산했다. 새 추론이나 품질 개선 작업 완료로 표현하지 않는다.
- Community→Sortformer DER: 일본54.760→18.577%이지만 누락 증가, 중국10.890→13.033%, 영어15.612→43.372%로 악화. 음량 보정도 일부 누락/오검출/혼동 회귀가 남는다. 한국어는 사람 정답 미완성으로 미측정이다.
- 결론은 네 언어 전체 개선 **미입증**. 좋은 언어/지표만 골라 주장하지 않으며 후보·평가 문턱·분모·운영 기본값을 변경하지 않았다. [주장 검증과 남은 조건](quality/four-language-claim-2026-09-20/REPORT.md)에 공개 집계·입력 SHA·재계산 코드를 저장했다.
- ca4ecbf의 필수 CI 전체 통과 확인. 새 음성/유료 호출/게시 없음. 다음은 한국어 사람 검수, 독립 개발 자료에서 회귀 없는 후보 확정, 사전 고정한 새 네 언어 여러 대화의 CER/문자 화자/DER와 기존11조건 검증이다. PR37 초안·자동화PAUSED 유지.

## 2026-09-20 사용자 계속 요청: 영어 사람 전사 연결·한국어 검수 자료

- AMI 공식 manual v1.6.2와 저장된 영어 row를 전체242구간/화자ID로 대조해 TS3003a 전사를 연결했다. 고정120초의191단어/801자이며 경계를 넘는2자는 항상 미확인으로 보존했다.
- 기존 예측 재사용, 사람 단어/시각으로 화자 배정만 진단: Community 정답695/오배정32/미확인74, Sortformer715/32/54. 전체 자동 ASR 정확도가 아니다. 단어 시각80% 미만만 덮는 정답이49/187자로, 짧은 겹침을 단어 전체 정답으로 보는 현재 배정 방식의 한계를 계측했다. 배정 알고리즘/기준 변경 없음.
- 한국어 OpenSLR113 공식 음원과 부분 전사를 확보했다. 첫 실제 s01m16f1의120초: 질문자 대사 누락22구간/특수표기27구간. 캐시 Whisper small 초안24구간과 비공개 REVIEW.html을 준비했다. 사람 검수/완전한 겹말 정답/네 언어 평가 완료가 아니다.
- Python597통과/7skip, 별도 DER23통과, 변경 Ruff 통과. Claude 검토로 시간 없는 토큰 처리, lock 구간/출처 선언 검증과 회귀를 보강했다. 실제 영어 채점을 수정 후 재실행해 동일 수치 확인. 시작 a26c626 CI 전체 통과; 이번 커밋 CI는 별도 확인 대상.
- [상세 보고서](quality/reference-link-2026-09-20/REPORT.md). 음원/대사/검수 화면은 `/Users/an-youwon/Projects/recording4-evaluation-private/reference-link-20260920`만 사용했다. 운영/새 유료 호출/게시 없음, PR37 초안 및 자동화PAUSED 유지.
- 다음: 한국어 질문자/겹말의 사람 정답 보완, 새 실제 STT와 독립 CER 연결, 독립 개발 표본에서 약한 단어 시각 근거 배정 개선 후 기존11조건·새 네 언어 비회귀 검사. 평가 표본에 맞춘 문턱 조정 금지.

## 2026-09-20 사용자 후속: 원인별 후보와 새 실제 대화 3언어 실측

- 원인별 수정 후보로 고정 음량 보정(최대20dB/-1dBFS peak 분석 사본), Sortformer 공식 긴 문맥 설정을 구현·시험했습니다. 긴 문맥은 기존 AMI 누락18.91초 그대로/DER50.72→51.93%로 악화해 채택하지 않았습니다.
- 새 중국어 AliMeeting Eval 첫 회의, 일본어 J-CRe3 test 첫 대화, 영어 AMI test row1을 각120초로 사전 고정했습니다. 기존/Sortformer×원음/사본 총 **12/12 실측 완료**. DER 원음→사본: Community 중국10.890→10.861%, 일본54.760→54.705%, 영어15.612→15.437%; Sortformer 중국13.033→12.202%, 일본18.577→16.569%, 영어43.372→43.072%.
- 평균 오류가 줄어도 Community 영어 혼동0.087→0.205초와 일본어 누락6.907→6.942초가 늘었습니다. Sortformer 일본어/영어 무음 오검출도 늘어 전역 적용하지 않았습니다. 일본어에서는 화자 혼동이 주요 원인으로 드러났지만 언어별 자동 모델 선택을 결정할 표본은 부족합니다.
- `prepare_dialogue_pilot.py`로 세 언어의 원본 SHA/화자 시각을 최초 입력과 동일하게 재현했습니다. DER 전용 불완전 코호트이며 full multilingual gate/문자 화자 지표를 우회하지 않습니다. 원시 자료는 `/Users/an-youwon/Projects/recording4-evaluation-private/multilingual-20260920`에만 보존했습니다.
- 한국어 KMSAV ASD43,805개 주석을 실제 점검했으나 문장 간 화자ID가 없어 화자 정답으로 사용할 수 없습니다. 대안 KEMDy20은 공식 배포본 WAV 미포함/협약 필요. 한국어 사람 화자 주석, 영어 대사 정답, 새 네 언어 CER/문자 화자 지표와 후보의 기존11조건 회귀는 미완료입니다. 네 언어 검증 완료라고 표현하지 않습니다.
- Claude 검토 반영: 음량 사본 처음부터0600 생성, PCM16 실제 peak 기록, JSON 직렬화 실패 시 불완전 증거 파일 방지. Python **594통과/6skip**, 변경 Ruff 통과. 시작 HEAD89063e8 모든 필요한 CI통과. 새 커밋은 별도CI 확인 대상.
- [실측/자료/재현/한계](quality/multilingual-pilot-2026-09-20/REPORT.md). 운영/유료 호출/게시 변경 없음, main PR#37 초안/자동화PAUSED 유지. 다음은 사람 정답 자료 보완과 개발 표본에서 화자 혼동·누락을 함께 줄일 후보를 검증하는 일입니다.

## 2026-09-20 사용자 후속: 불일치 검수 → DiCoW → 누락 진단

- 선택한 계획을 우선 실행했습니다. 자동화는 PAUSED 유지이며 수동 후속 작업입니다. 운영 모델/기존 화자 배정은 변경하지 않았습니다.
- 두 모델의 정답 없는 시간 대응으로 검수 표시/겹말 재시도 창을 생성했습니다. 같은 11조건 4,389자의 오배정 109자 모두 표시되지만 정답 212자도 표시됩니다. 기존 정답/오배정/미확인 3767/109/513 그대로이며 자동 수정 0입니다.
- **GPU 결제 없이 Mac CPU float32 DiCoW 8/8 완료**. 같은 환경에서 50% 겹말 CER 159/399(39.85%) → 6/399(1.50%), 25%는 2/399 → 3/399로 악화했습니다. 이 결과는 고정 겹말 클립의 대사 인식이며 전체/다국어/화자 정확도 개선으로 일반화하지 않습니다. 18개 국소 재시도 창은 준비만 했으며 그 창으로 이 점수를 계산하지 않았습니다.
- AMI 저장 예측의 누락 원인을 고정 20ms 음향 층으로 진단했습니다. 전체 누락 7.4365초 → 18.91초, 짧은 발화는 1.3207 → 0.73초로 오히려 개선되어 긴 발화 누락이 남습니다. 혼합 음량만으로 인과를 단정하거나 평가 자료에 맞춰 튜닝하지 않았습니다.
- 새 한국어 KMSAV/일본어 J-CRe3/중국어 AliMeeting의 공식 자료 조건을 조사하고 한·일 목록의 리비전/해시를 비공개 고정했습니다. 실제 음원·사람 정답 변환, 새 영어 자료, 네 언어 lock/채점은 아직 미완료입니다.
- 검증: Python **577통과/6skip**, 신규 파일 Ruff 통과. 시작 HEAD 89f94a9 CI 전체 통과. 현재 변경 CI는 push 후 별도로 확인합니다. 새 native 실행 래퍼는 help만 확인했고 실제 모델 실행은 기존 benchmark 직접 호출이므로 래퍼 전체 검증으로 표현하지 않습니다.
- Claude 최종 검토 반영: 검수 입력의 동일 음원 SHA/선언 길이 확인, 재비교의 오래된 사유 제거. 진단 최대 8화자/긴 입력 비용은 실험 제한으로 기록했습니다.
- 상세 수치/해시/한계: [보고서](quality/disagreement-2026-09-20/REPORT.md). 원시 자료는 비공개 평가 폴더만 사용하고 다른 작업자의 .runtime은 읽기만 했습니다.
- 다음: 독립 개발 대화에서 긴 발화 누락 수정, 국소 창별 DiCoW 검증 및 독립 음성·시각 근거 연결, 실제 새 네 언어 평가. 25%/AMI 회귀와 기존 CJK·장시간 검토가 남아 main PR #37은 초안 유지합니다.

## 2026-09-20 12:33 KST 자동화 종료

지정 종료 시각이 지나 recording4 자동화를 **PAUSED**로 변경했습니다. 마지막 코드90a3239 로컬 Python562통과/6skip, Ruff통과. 최신 CI는 웹 통과/Python·DER 대기이며 완료로 표현하지 않습니다. main PR #37 초안 유지. 별도 사용자 GPU 대여 요청은 아래 계획까지 준비했고 Mac 잠금 해제와 새 지출 승인 전 서버 생성/결제/업로드 없음.

## 2026-09-20 12:32 KST: 사용자 단기 GPU 대여 요청

- 사용자가 단기 GPU 서버 실험을 요청했습니다. 기존 자동화 종료와 별개의 수동 후속 요청이며 서버 생성/결제/업로드는 아직 없습니다.
- CUDA 명시 선택/모델·텐서 이동, CPU INT8 병용·GPU 부재 거부, CPU/GPU 증거 지문 분리, Docker GPU/24GiB 옵션을 준비했습니다. CPU 기본/기존 지문은 유지합니다. 실제 CUDA 실행은 미검증입니다.
- 검증: Python **562통과/6skip**, 변경 파일 Ruff 통과. 실제 GPU 모델/실시간 결제는 미실행.
- Mac이 잠겨 계정/실시간 견적 화면 확인이 막혔고 사용자에게 잠금 해제를 요청했습니다. Runpod 선불 최소 $10/환불불가는 기존 월 예산과 별도 승인 필요. 구체 제안·비교 입력·종료 과금 주의는 [GPU 계획](quality/gpu-evaluation-2026-09-20/PLAN.md).
- 다음: 사용자 잠금 해제/지출 승인 뒤 실제 견적 확인 및 종료 제어 준비 → 기존/후보 모두 GPU float32로 같은 4입력 비교. CPU INT8 대비 차이를 GPU 화자 개선이라고 혼동하지 않습니다. 원시 대사·음원·자격증명은 GitHub에 저장하지 않습니다.

## 2026-09-20 12시대: 사용자 요청 Sortformer 실제 비교

- 사용자 “그렇게 해” 요청으로 공식 NeMo Sortformer v1/v2.1을 Mac 별도 CPU 환경에 설치하고 AMI 1개+기존 11조건을 각각 실행(총 24회), 기존 Community-1 결과와 비교했습니다. 현재 워커가 이미 Community-1인 것을 소스로 확인했습니다. 기존 운영과 다른 작업자 `.runtime`은 변경하지 않았습니다.
- 4,389자 정답/오배정/미확인: 기존 3767/109/513 → v1 **3860/0/529**, v2.1 **3848/0/541**. 50% 겹말은 개선됐지만 25% 겹말 정답 일부 감소. AMI는 두 후보 모두 4명을 찾았으나 DER **34.4878% → 50.3597%/50.7194%** 악화. 조건별 비회귀/새 실제 대화 기준을 통과하지 못해 운영 교체 보류.
- v2.1의 예측 마스크를 기존 DiCoW 4입력에 연결했으나 첫 결과 전 종료137, **0/4 완료**. 7GiB 제한에서 메모리 부족 의심, OOM 확증 없음. CER 개선 주장/분모 축소/맹목 재시도 없음. 기존 DiCoW 결과 보존. 재시도 전에 메모리 계측/자원 확보가 필요합니다.
- 추론 CLI `run_sortformer_evaluation.py`, Mac 평가 의존성 잠금, 결과 파서 16검사 추가. Python **560통과/6skip**, Ruff 통과. 코드/실측은 [비교 보고서](quality/sortformer-2026-09-20/REPORT.md). v1 가중치 CC BY-NC 연구용; v2.1 NVIDIA Open Model License. 원시 음원/대사/예측은 비공개 평가 폴더에만 보존.
- 다음: 이 변경의 CI 확인, DiCoW 종료137 원인 계측, 독립 개발 음성에서 발화 누락 개선, 새 한·일·중 실제 대화 검증. 이번 표본에 맞춘 임계값 튜닝/조건별 최선 결과 선별은 하지 않습니다. main PR #37은 초안 유지. 기존 자동화 종료 **12:32:10 KST**는 유지하며 마지막 실행은 정리/PAUSED만 수행합니다.

## 2026-09-20 11시대: 실제 영어 회의의 고정 구간 DER 실행

- `cba3a9b` CI 전체 통과 확인. 기존 AMI ihm/test 첫 행 앞 120초를 원본 보존 사본에서 오프라인 CPU로 한 번 실행해, 과거 집계만 있던 자료의 모델 예측 구간을 새로 비공개 저장했습니다. 132.04초 소요, 새 유료 호출/모델 다운로드/운영 변경 없음.
- 새 score_segments 고정 UEM 0–120/collar0/겹말포함 결과: **DER 34.4878%**, 정답 화자 4명/예측 2명, 누락 7.4365초·혼동 4.8671초·오검출 1.1190초/분모 38.92초. 과거 암묵적 UEM 계산도 같은 저장 예측에서 같은 값. **정확도 개선 없음**.
- 이것은 영어 한 표본 진단이며 네 언어 코호트 검사를 통과한 결과가 아닙니다. 원래 주석에 대사가 없어 완전 manifest에 가짜 값을 넣지 않았습니다. 코호트/평가 기준 변경 없음. 기존 11조건 재검사도 반복하지 않았습니다.
- 비공개 예측·사본 보존: `/Users/an-youwon/Projects/recording4-evaluation-private/ami-der-20260920`. 다음 채점은 저장 예측을 사용하고 모델을 재실행하지 않습니다. 원본 SHA 보존 확인. [상세 수치/출처/제한/재현](quality/ami-der-2026-09-20/REPORT.md).
- 운영 코드 수정 없음. Python/DER 검사는 이전 통과 상태 유지. main PR 초안 유지. 다음 종료 실행(12:32:10 KST)은 최신 CI/변경/남은 일을 저장하고 자동화를 PAUSED로 바꿉니다.

## 2026-09-20 10시대: 고정 구간의 독립 DER 채점 연결

- PR #37의 `c04cf15` CI 전체 통과 확인. `score_conversation_diarization.py`를 추가해 현재 실제 파일/lock 재검증 후 저장된 모델 화자 구간을 사람 정답과 pyannote.metrics로 채점합니다. collar 0·겹말 포함·명시적 전체 평가 구간 UEM, 실패 분모 보존, 언어별/전체 시간 가중 집계를 적용합니다.
- pyannote.metrics 3.2.1/core 5.0.0 전용 환경을 `/tmp/r4-der-env`에 설치하고 라이브러리 실제 검사 **17통과**. 잠금 의존성 파일과 CI 독립 DER 작업 추가. 기본 Python 전체 **544통과/6skip**(평가 라이브러리 없는 환경의 DER 모듈 skip 1 포함), Ruff 통과. DER 17개는 별도 환경에서 실제 실행했으며 미실행으로 남기지 않았습니다.
- 손으로 계산한 fixture 검증: 완전 일치/이름 변경 DER 0, 겹말 누락+무음 오검출 0.75, 빈 실패 예측 1.0. 중복 같은 화자 트랙이 분모를 부풀리지 않는 것도 확인했습니다. 실제 새 대화 품질 개선 실측이 아닙니다.
- Claude 검토 후 화자 혼동 2초/4초=DER 0.5와 입력 SHA/조건/상태 방어 7검사를 추가하고 조건별 평가 구간을 결과에 기록했습니다. prepare()가 비어 있는 코호트를 이미 거부하므로 0 분모 지적은 이 경로에 해당하지 않았습니다. 고정 입력 구간 엄격 검사와 비공개 오류 로그 방어는 유지했습니다. 39eb0dd의 새 DER CI도 통과 확인했으며 후속 커밋은 새 CI로 확인할 것.
- 새 모델 추론/유료 호출/운영 변경 없음. 실제 한·일·중 대화 자료/모델 예측 연결, CER/문자 화자 채점, CJK/장시간 처리와 50% 기준 목소리 과제는 남습니다. 종료 12:32:10 KST 유지. 형식/재현은 대화 평가 PLAN.md 참고.

## 2026-09-20 09:36 KST: 기존 품질 검사 CLI의 입력/증거 보존

- PR #37의 `68fd093` Python·웹·워커 이미지·스택 CI 전체 통과 확인. 기존 `check_speaker_quality.py` CLI에 남은 Claude 검토 지적을 반영했습니다.
- UTF-8 및 엄격 JSON 로더(중복 키·비유한 수 거부) 적용. 없는 파일/잘못된 입력은 고정 invalid 이유와 종료 1로 처리해 예외의 경로/원문을 출력하지 않습니다. invalid 결과는 기존처럼 delta/cases가 없는 별도 상태이며 소비자는 verdict/종료 코드를 먼저 확인해야 합니다.
- 같은 결과는 재사용하지만 다른 기존 결과는 덮어쓰지 않습니다. 정상 결과에 기준/후보 파일 SHA를 추가해 변화량이 같아도 입력이 달라지면 새 출력 경로를 요구합니다. 비교 기준과 evaluate() 반환 계약은 변경하지 않았고 이를 직접 호출하는 자동 검수/새 코호트 비교기의 저장 계약도 그대로입니다.
- 새 CLI 회귀 6개 포함 Python **544통과/5skip**, Ruff 통과. 실제 음성 모델/음원 검사를 반복하지 않았으며 정확도 향상 실측이 아닙니다. main PR 초안/운영 기본 유지.
- 다음: 새 실제 대화 자료·독립 채점 연결, CJK/장시간 처리 검토. 이번 자동화 종료 12:32:10 KST 유지.

## 2026-09-20: 사용자 요청으로 작업 재개 — 고정 코호트 점수 비교

- 사용자의 '계속' 요청으로 작업과 후속 자동화를 재개했습니다. 이전과 같은 최대 4시간으로 설정했으며 이번 종료는 **2026-09-20 12:32:10 KST**입니다. 마지막 실행은 저장/보고 후 PAUSED로 변경합니다.
- `compare_conversation_evaluation.py` 추가: 실제 manifest/음원/정답을 재검사한 lock과 양쪽 점수 파일을 연결합니다. 해시·언어·분모·전체 조건 범위를 확인하고 기존 정답 증가/오배정 비증가 규칙을 적용합니다. 실패/거부도 분모에 남기고 후보에 남은 실패가 있으면 개선 판정을 거부합니다. 언어별 수치를 출력하고 기존 결과 덮어쓰기를 차단합니다.
- 실제 CLI 회귀에서 정상 비교 후 음원을 변경하면 실패 코드 1, 기존 결과 보존 확인. 새 비교 검사 14개와 입력 검사 2개, Python 전체 **538통과/5skip**, Ruff 통과. 테스트 데이터는 합성 fixture이며 실제 새 음성 정확도 측정은 아닙니다.
- Claude 검토 반영: 기준 실패를 후보가 회복해도 거부되던 비교 규칙을 수정하고 회복 회귀 검사 추가. WAV sample rate 0 방어와 정책 객체 복사도 추가. 기존 check_speaker_quality.py 독립 CLI의 UTF-8/엄격 JSON/덮어쓰기 정책 점검은 후속 작업으로 남깁니다. 새 비교기는 이미 엄격 입력/덮어쓰기 방어를 적용합니다.
- 아직 미연결: 실제 한·일·중 대화 자료/사람 검수, 모델 실행 및 독립 채점·DER/CER 생성. 비교기의 점수 입력은 신뢰된 채점 결과가 필요합니다. 운영 기본/유료 호출/게시 변경 없음.

## 2026-09-20 07:15 KST: 4시간 자동화 종료

사용자가 지정한 종료 시각 07:14:35 KST가 지나 새 개선 작업을 시작하지 않고 종료 정리했습니다. `recording4` 자동화는 PAUSED로 변경했습니다.

- 병합: PR #35·#38 → #34 브랜치, PR #34 → 원래 개발 브랜치(`f7f2a2b`). main 통합 [PR #37](https://github.com/plutoan12/recording4/pull/37)은 초안 유지입니다. main 배포 완료가 아닙니다.
- 마지막 코드 `070ea4a`에서 GitHub Python·관리화면·워커 이미지·스택 CI 모두 통과 확인(run 35471484158). 로컬 최종 Python 522통과/5skip, Ruff 통과. 이 종료 기록 이후의 문서 커밋 CI는 별도이며 코드 검사를 반복하지 않았습니다.
- 완료: 화자 분석 사전조건 오류/대본 버전 중복 요청 방어, 싱크 지지 근거 표시, FFmpeg 실행 제한, 새 다국어 실제 대화 평가 입력 고정과 잘린 파일/잘못된 JSON 방어. Claude 검토 중 실제 확인된 문제를 반영했습니다.
- 남은 구현·검토: CJK 단어 경계, 크롭 부분 실패 복구와 긴 영상 비용, 실제 한·일·중 대화 자료 및 사람 검수, 고정 코호트를 연결한 기준·후보 비교 실행, 50% 겹말 기준 목소리 확보, 대상 ASR의 독립 음성·시각 검증.
- 미해결 품질: 기존 11조건 4,389자의 정답/오배정/미확인 3,767/109/513, 싱크 140조건 112통과/28거부. 이번 자동화에서 새 실음성 정확도 향상을 실측하지 않았습니다. 주관적 음질 청취와 네 언어 실제 대화 일반화도 완료로 표현하지 않습니다.
- 이번 자동화 중 새 유료 호출·구독·게시·운영 재배포 없음. 다른 작업자 폴더와 비공개 runtime 보존. 이후 재개는 이 작업 폴더와 PR #37의 최신 상태부터 확인합니다.

## 2026-09-20 06:48 KST: 평가 입력 오류와 잘린 WAV 차단

- PR #37의 `707348e` CI 전체 통과 확인. 후속 평가 입력 점검에서 최상위 JSON이 객체가 아닐 때 traceback이 나오던 경로와, WAV 헤더만 정상인 잘린 PCM 파일이 준비 완료로 처리되던 경로를 수정했습니다. 비정상적으로 큰 정수 시각도 고정된 입력 오류로 거부합니다.
- WAV payload를 제한된 크기로 읽어 선언된 프레임 바이트 수와 비교합니다. 모델 추론·새 유료 호출 없이 불완전 입력만 차단합니다. 원시 데이터는 저장소에 추가하지 않았습니다.
- 새 회귀 검사 5개 포함 Python **522통과/5skip**, Ruff 통과. 실제 음성 정확도 수치 변화 없음. main PR #37은 초안 유지, 운영 재배포 없음.
- 다음 마지막 실행은 07:14:35 KST 이후 새 개선 작업 없이 최신 CI/작업 상태를 기록하고 자동화를 일시 중지합니다. 실제 다국어 대화 자료/비교 실행기, CJK 경계, 50% 기준 목소리와 독립 음성·시각 검증은 남습니다.
...
## 2026-09-22 사람 음성 검수 화면 준비

- 15개 후보 WAV를 브라우저에서 재생하고 화자·겹말·확신도·검수자 정보를 기록해 `labels.reviewed.json`으로 내려받는 로컬 화면을 비공개 평가 폴더의 `review-pack-20260921/review.html`에 생성했다. 라벨은 같은 브라우저의 localStorage에 임시 저장되며, 화면에는 자동 전사와 모델 화자 결과를 노출하지 않는다.
- 실행: Finder에서 `review.html`을 열면 된다. 원시 WAV·검수 자료는 저장소에 넣지 않았다. 내려받은 JSON은 아직 사람 정답으로 검증되거나 채점기에 승인된 자료가 아니다.
- 남은 단계: 검수자가 15개를 듣고 라벨을 기록·내보낸 뒤 원본/라벨 스키마와 SHA를 재검증하고 같은 분모로 채점한다. EgoBlur 전체 영상 검증에는 Linux/CUDA 환경이 별도로 필요하다.

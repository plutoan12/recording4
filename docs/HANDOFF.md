...
- pyannote.metrics 3.2.1/core 5.0.0 전용 환경을 `/tmp/r4-der-env`에 설치하고 라이브러리 실제 검사 **10통과**. 잠금 의존성 파일과 CI 독립 DER 작업 추가. 기본 Python 전체 **544통과/6skip**(평가 라이브러리 없는 환경의 DER 모듈 skip 1 포함), Ruff 통과. DER 10개는 별도 환경에서 실제 실행했으며 미실행으로 남기지 않았습니다.

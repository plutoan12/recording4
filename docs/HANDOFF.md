...
위 열 조건에 없던 모양을 만들어 쟀습니다. `--drift 0.15`는 자막마다 어긋남을 0.15초씩 키웁니다(첫 자막 0초, 둘째 0.15초, 셋째 0.30초…). 자막 9개면 첫 자막과 마지막 자막이 1.2초 벌어집니다. **이동값이 하나뿐인 통째로 옮기기는 원리상 고칠 수 없는 모양**이고, 강제 정렬을 제안한 이유가 바로 이것입니다.
...
**잰 조건에서는 여전히 통째로 옮기기가 이깁니다**(0.780 대 1.012). 열한 번째 조건에서도 졌습니다. 다만 이 표는 앞의 표와 성질이 다르고, 처음으로 두 가지가 드러났습니다.

**하나. 두 방법이 서로 다른 것에 흔들립니다.** 잡음 15dB에서 어긋남이 없을 때 통째로 옮기기는 0.000초였습니다. 자막당 0.15초를 주자 0.780초가 됐습니다. 같은 조건에서 단어 정렬은 1.012초 → 1.012초로 **값이 그대로입니다.** 짧은 발화 간격(0.809/0.809)과 빠른 말(0.765/0.765)도 이동 0초와 2.5초에서 같은 값입니다. 단어 정렬은 원래 자막의 시각을 쓰지 않고 음성에서 다시 찾으므로 **입력이 얼마나 틀어졌든 오차가 같습니다.** 통째로 옮기기의 오차는 입력이 틀어진 모양을 따라 커집니다.

자막 9개에 자막당 d초씩 쌓이면 이동값 하나로는 최소 4d초가 남습니다. 단어 정렬의 바닥이 약 1.0초이니 **계산으로는 d가 0.25초쯤에서 뒤집힙니다. 계산일 뿐 아직 재지 않았습니다.**

**둘. 어긋남이 쌓이면 우리 안전장치가 통째로 옮기기를 먼저 거부합니다.** 짧은 발화 간격과 빠른 말에서 보정값이 -1.100초·-1.210초로 나왔습니다. 첫 자막이 1.08초에 있으니 이 보정은 자막을 원본 시작 앞으로 밀어냅니다. 그런 보정은 쓰지 않기로 했으므로 거부됩니다. 상호상관은 **평균** 어긋남만큼 당기는데 앞쪽 자막에는 그만큼의 여유가 없기 때문입니다. 이때 **단어 정렬이 결과를 내는 유일한 방법입니다.** 거부는 안전한 동작이지 좋은 결과가 아닙니다. 사람이 손으로 고쳐야 합니다.

### 어디서 뒤집히는가 (CI run #328) — 뒤집혔습니다

8초 음악 도입 조건 하나에서 자막당 어긋남을 0 / 0.15 / 0.3 / 0.45 / 0.6초로 올리며 쟀습니다. 이 조건은 앞에 8초 여유가 있어 보정값이 자막을 원본 시작 앞으로 밀어내지 않습니다.
...
**자막당 0.30초부터 단어 정렬이 이깁니다.** 앞서 계산으로 짚은 0.25초와 가깝습니다. 단어 정렬은 열 칸 모두 1.535초로 **완전히 같습니다**(어긋남 다섯 값 × 이동 두 값). 원래 자막의 시각을 아예 쓰지 않는다는 것이 숫자로 확인됐습니다.

### 그런데 표본이 바뀌었습니다 — 실행 사이 비교는 무효입니다

위 표를 앞의 표들과 나란히 놓을 수 없습니다. 같은 이름의 조건(music_intro)을 run #306에서는 통째로 옮기기 0.040 / 단어 정렬 1.012로 쟀습니다. 길이도 100.771초 대 100.832초로 다릅니다.

로그를 끝까지 보니 **받은 문장 아홉 개가 전부 다릅니다.** 같은 `--offset 30`으로 30분 간격을 두고 받았는데 데이터셋이 다른 행을 돌려줬습니다.

- #306: "한 온라인 영어학원은 여자 아이돌 그룹의 앨범 재킷…" (앞 무음 0.08초, 잡음 바닥 -35dB)
- #328: "하지만 때때로 해로운 병균 때문에 몸에 이상을…" (앞 무음 0.00초, 잡음 바닥 -46dB)

녹음의 성격 자체가 다릅니다. 그러므로:

- **한 실행 안의 비교는 유효합니다.** 위 열 칸은 모두 같은 음성입니다. 뒤집힌다는 사실은 남습니다.
- **실행 사이의 비교는 무효입니다.** 0.530과 0.040은 다른 음성의 값입니다. 뒤집히는 지점 0.30초도 표본 하나에서 한 번 잰 값입니다.

이전에 전사 검증(CER)에서 "그날 뽑힌 표본을 재고 있다"고 적었습니다. 같은 문제가 **싱크 측정에도 있었습니다.** `--offset`으로 고정된다고 본 것이 틀렸습니다.

### 두 표본에서 같은 지점에 뒤집힙니다 (CI run #330)

표본 id를 붙여 다시 돌렸더니 데이터셋이 **예전 표본을 도로 돌려줬습니다**(id `095fd58c31ba`, 길이 100.771초 — run #306과 같습니다). 덕분에 같은 측정을 서로 다른 두 표본에서 각각 한 셈이 됐습니다.
...
**두 표본 모두 자막당 0.30초에서 뒤집힙니다.** 절대값은 표본마다 다르지만(단어 정렬의 바닥 1.012초 대 1.535초) 뒤집히는 지점은 같습니다. 한 번 잰 값이 아니라 두 번 잰 값입니다.

`095fd58c31ba`의 어긋남 0 값(0.040 / 1.012)은 열 조건 표의 「8초 음악 도입」 줄과 정확히 같습니다. **그 표가 이 표본에서 나왔다는 것이 확인됐습니다.**

### 표본 고정이 먹습니다. align은 같은 입력에서도 0.01초 흔들립니다 (CI run #332)

`--expect`를 걸고 다시 돌렸더니 두 검사 모두 기록해 둔 표본을 찾았습니다("기록해 둔 표본을 0번째 쓸 수 있는 행에서 찾았습니다", 표본 id `095fd58c31ba` / `a782da697dd0`).

같은 표본에서 같은 코드로 두 번 잰 값입니다.
...
**통째로 옮기기는 소수점까지 같습니다. 단어 정렬은 0.010초 움직입니다.** 전사 검사도 같은 표본에서 CER 10.4% → 9.7%로 움직였고 글자도 달라졌습니다("위쪽되면서" → "위촉되면서"). CPU에서 도는 신경망 쪽 재현성 문제로 보입니다.

**그래서 단어 정렬 값은 0.01초 단위로 견주지 않습니다.** (이 0.010초는 조건 하나만 보고 적은 값이고, 아래 「정정」 절에서 0.141초로 바로잡았습니다. 기준선도 0.1초에서 0.2초로 옮겼습니다.)

### 열 조건을 고정 표본에서 다시 쟀습니다 — 값이 그대로입니다 (CI run #338)

표본 `095fd58c31ba`에 고정하고 열 조건을 한 번에 다시 쟀습니다.
...
**스무 칸이 모두 앞서 기록한 표와 소수점까지 같습니다.** 섞였을까 걱정한 표가 실은 한 표본에서 나온 것이었습니다. 이제 그 표본이 무엇인지도 적혀 있습니다. 판정은 `--gate shift`로 걸었고 열 조건 모두 0.5초 한계 안입니다.

### 「단어 정렬이 이긴다」가 「쓸 만하다」는 뜻은 아닙니다

어긋남이 쌓이는 경우 단어 정렬이 이기지만 **두 방법 다 0.5초 한계를 넘습니다.**
...
단어 정렬은 **덜 틀릴 뿐입니다.** 자동으로 맞는 자막이 되지는 않고 사람이 손봐야 합니다. 화면 문구·API 문서·설정 주석에 이 문장을 적었습니다.

### 단어 정렬의 0.75초는 첫 자막에 몰려 있습니다 (미해결)
...
**둘째·셋째는 0.08초 안으로 맞고 첫째만 0.77초 늦습니다.** VAD도 같은 지점을 1.86초로 잡습니다. 세 조각 모두 앞에 1초 무음이 있으니 위치 때문은 아니고 첫 조각의 내용 때문으로 보입니다.

**여기서 멈췄습니다.** 정답 시각은 에너지 문턱으로 잰 값이고, 그 문턱에 맞춰 붙이는 고침을 만들면 정답에 맞춰 코드를 고치는 것이 됩니다. 그러면 검사가 검사를 통과시킵니다. 실제로 어느 쪽이 맞는지는 사람이 그 한 조각을 들어 봐야 합니다. 사실만 적고 고치지 않았습니다.

### 첫 자막 가설은 기각됐습니다 (CI run #345)

열 조건에서 **첫 자막을 뺀 최대 시작 오차**를 함께 쟀습니다. 읽은 아홉 줄입니다(`short`는 로그 창 밖이었습니다).
...
**아홉 줄 모두 두 값이 같습니다.** 첫 자막을 빼도 최대 오차가 줄지 않습니다. 사람 목소리 세 문장에서 본 0.77 / 0.08 / 0.07초는 **그 음성의 모습이었지 강제 정렬의 일반적인 성질이 아닙니다.** 「첫 자막만 고치면 된다」는 길은 닫혔습니다.

### 정정: align의 실행 간 흔들림은 0.01초가 아니라 0.14초입니다

앞에서 "align은 같은 입력에서 0.010초 움직인다"고 적었습니다. **조건 하나만 보고 한 말이었고 틀렸습니다.** 열 조건을 두 번 돌려 견주면 잡음 15dB에서 1.012 → 0.871로 **0.141초** 움직입니다. shift는 아홉 줄 모두 소수점까지 같습니다.

그러므로 align 값은 **0.2초보다 작은 차이로 견주지 않습니다.** 그리고 그 결과 **뒤집히는 지점 0.30초/자막은 확정값이 아닙니다.** 그 지점의 차이(1.220 대 1.022 = 0.198초)가 방금 본 흔들림과 같은 크기입니다.

확실한 것은 이렇습니다.

- 자막당 **0.15초**: 통째로 옮기기가 이깁니다(0.790 대 1.012, 차이 0.222초).
- 자막당 **0.45초 이상**: 단어 정렬이 이깁니다(1.980 대 1.012, 차이 0.968초).
- 그 사이 어딘가에서 뒤집히고 **0.30초는 지금의 최선 추정**입니다.

### 결론

두 방법은 **서로 다른 일을 잘합니다.**

- 한 덩어리로 어긋난 경우 → 통째로 옮기기(0.000~0.500초 대 0.75~2.63초)
- 자막마다 0.30초 이상씩 쌓이는 경우 → 단어 정렬
- 그 사이(0.15초 언저리) → 통째로 옮기기가 여전히 낫습니다

**기본값은 그대로 통째로 옮기기입니다.** 실제 자막이 어느 쪽 모양으로 어긋나는지는 아직 재지 않았습니다. 흔한 원인(녹화 시작 시각 차이, 앞부분 잘라내기)은 한 덩어리 어긋남입니다. 자막당 0.30초씩 쌓이면 자막 9개에서 2.4초가 벌어져 사람이 먼저 알아차립니다. 고를 수 있게 두고 어느 쪽인지는 사람이 봅니다.

**한국어 낭독 싱크 정확도 자체는 올리지 못했습니다.** 한 덩어리 어긋남에서 최선은 여전히 통째로 옮기기의 0.50초입니다. 이번에 얻은 것은 "단어 정렬이 쓸모 있는 자리가 어디인지"입니다.

### 고침: 표본 id

- `fetch_korean_speech.py`가 **표본 id**(문장 목록 sha256 앞 12자)를 만들어 `expected.json`에 남기고 화면에 찍습니다. 행 번호가 아니라 **실제로 쓴 문장**에서 만듭니다.
- `verify_sync_matrix.py`가 그 id를 `results.json`과 표 위아래에 찍습니다. **표본 id 없는 표는 견주지 않습니다.**
- `--expect <id>`를 주면 행 순서가 바뀌어도 앞 500행을 훑어 그 표본을 찾습니다. 찾지 못하면 실패합니다. 조각 하나만 못 받아도 실패합니다. 하나가 빠지면 다른 표본이고, 그러면 id가 거짓말을 합니다.
- 음성은 커밋하지 않습니다. id는 문장에서 나온 값이라 음성 없이도 "같은 표본인가"를 물을 수 있습니다.

**다음 실행이 표본 id를 찍습니다.** 그 값을 CI에 박고, 고정된 표본에서 뒤집히는 지점을 다시 재야 합니다. 그 전까지 0.30초는 **한 번 잰 값**입니다.
...
- CI가 이제 표본을 고정합니다. 싱크 행렬은 `--expect 095fd58c31ba`, 사람 목소리 검사(정렬·전사·화자 분리)는 `--expect a782da697dd0`입니다. 데이터셋이 그 표본을 앞 500행에서 내주지 못하면 다른 표본으로 갈아타지 않고 실패합니다.
- 전사 검증(CER)이 그날 표본에 따라 10.4%와 54.8%를 오가던 문제도 이 고정으로 닫혔습니다. 한계값은 건드리지 않았습니다.
- 앞의 열 조건 표는 표본 id가 없던 때의 값입니다. run #306과 #314가 같은 문장 아홉 개를 쓴 것은 로그로 확인했지만, 그보다 앞선 실행(#288·#296·#302)은 확인하지 않았습니다. 표는 남기되 **표본이 같다고 말하지 않습니다.**
- align의 자막 경계는 남은 문제입니다. 전체 위치는 0.1초 안으로 맞히므로 `cues_for_lines`가 단어를 줄에 묶는 방식이 의심됩니다. `snap_starts`는 껐다 켜서 재 봤고 기각됐습니다(-0.076~+0.022초).

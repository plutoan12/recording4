...
- 음성 기준은 기존 0.6 유사도/0.15 경쟁 margin을 유지한다. 시각 근거는 활성 화자 탐지와 사람 영상 검수를 구분하며 0~1 점수와 개발 후보 문턱 0.7을 검사한다. 다른 대상 근거, 복수 대사 후보, 오래된 음원, 다른 단어, 비유한/범위 밖 점수는 거부한다.
...
- 실제 영상 활성 화자 출력이 아직 없어 정확도 개선 실측은 없다. 관련 51검사, 전체 Python **674통과/20skip**, 전체 Ruff 검사·형식 검사가 통과했다. CI는 새 HEAD에서 확인한다. [계약과 한계](quality/target-independent-evidence-2026-09-20/REPORT.md). PR37 초안과 운영 기본을 유지한다.

...
이 작업 환경에서는 강제 정렬을 돌릴 수 없습니다. `huggingface.co`가 막혀 **음성 인식 모델도, 한국어 표본도** 받지 못합니다(`download.pytorch.org`도 막혀 CPU torch 대신 CUDA 빌드를 쓰지만, nvidia 라이브러리 경로를 잡아 주면 임포트는 됩니다).

**CI에는 둘 다 있습니다.** 워커 이미지에 CPU PyTorch와 stable-ts가 들어 있고, 러너는 Hugging Face에서 표본을 받습니다. 그래서 `verify-align` 라벨에 **싱크 방법 비교 (사람 목소리)** 단계를 붙였습니다.

- 한국어 사람 목소리 9개를 받아 `verify_sync_matrix.py --method both`로 **같은 조건에서 두 방법**을 잽니다.
- 조건은 `short,fast,dense`입니다. 통째로 옮기기가 약한 자리(짧은 영상 0.40초, 1.25배 빠른 말 0.50초)와 이미 잘 맞는 자리(짧은 발화 간격 0.03초)를 골랐습니다. 10가지를 둘 다 돌리면 이 잡의 시간 예산(45분)을 넘습니다. 그래서 60분으로 늘리고도 세 가지만 돌립니다.
- 판정은 `--gate shift`로 **지금 쓰는 방법에만** 겁니다. align은 재는 중이지 보증하는 것이 아닙니다. 재는 것과 보증하는 것을 섞으면 채택하지도 않은 방법 때문에 CI가 빨개집니다.
- 조건별 최대 오차를 한 표로 찍고 `results.json`에 남깁니다.

### 측정 결과 (CI run #256, 2026-09-19) — **align이 졌습니다**

| 조건 | 길이(초) | shift 최대 오차 | align 최대 오차 |
|---|---:|---:|---:|
| 짧은 영상 | 36.2 | **0.400** | 3.097 |
| 짧은 발화 간격 | 85.1 | **0.030** | 2.637 |
| 1.25배 빠른 말 | 74.2 | **0.500** | 1.630 |

align은 세 조건 모두 한계(0.5초)를 넘었습니다. 통째로 옮기기가 가장 약한 자리에서도 3배 나빴습니다. **제 가설이 틀렸습니다.**

다만 데이터가 한 가지를 더 말해 줍니다. align의 오차가 **이동 0초와 2.5초에서 똑같고**(3.097 / 2.637 / 1.630), align이 찾은 전체 이동값은 -2.41~-2.43초로 정답 -2.5초에 0.1초 안입니다. 즉 **전체 위치는 맞히는데 자막 하나하나의 경계가 어긋납니다.** 정렬 자체보다 단어 시각을 줄로 묶는 자리(`cues_for_lines`·`snap_starts`)를 먼저 봐야 합니다.

`정렬 품질 검증 (사람 목소리)`이 같은 날 0.77/0.08/0.07초를 낸 것과 어긋나 보이지만 재는 방법이 다릅니다. 그쪽은 **시작 시각만**, "가장 가까운 실제 발화"와 견줍니다. 행렬은 자막마다 **시작과 끝을 제자리에서** 봅니다. 느슨한 기준의 숫자를 엄격한 기준의 숫자와 나란히 두면 안 된다는 예입니다.

### 파 본 것 — 어긋난 곳은 끝 시각이고, 비교가 공정하지 않았습니다

시작이 0.08초인데 합친 오차가 2.6초라면 어긋난 곳은 **끝**입니다.

이유는 이 저장소가 이미 재 두었습니다(아래 「자막 끝 시각」 절). 정렬이 찾는 끝은 **말이 끝나는 지점**이고 정답으로 쓰는 끝은 **에너지 문턱**이라 말끝 숨소리·잔향만큼 늦습니다(1.21 / 1.41 / 1.74초). 그때 결론이 "정렬과 VAD가 서로 맞고 정답 끝만 늦다"였습니다.

행렬 검증의 정답 끝도 같은 에너지 문턱에서 나옵니다. **통째로 옮기기는 원래 자막 길이를 그대로 옮기므로 그 정답과 저절로 맞습니다**(끝 오차 ≡ 시작 오차). 강제 정렬은 끝을 음성에서 다시 찾으니 그 1.2~1.7초를 고스란히 벌로 받습니다. **끝 시각으로 견준 것이 공정하지 않았습니다.**

고친 것 두 가지입니다.

- 행렬 검증이 **시작 오차와 끝 오차를 나눠** 찍습니다(`max_start_error_seconds`, `max_end_error_seconds`). 합쳐 놓으면 어디가 어긋났는지 안 보입니다.
- `realign_subtitles`가 정렬기에서 **시작만** 가져오고 **자막 길이는 원래 것을 지킵니다**(`keep_duration`, 기본 켜짐). 통째로 옮기기가 길이를 보존하는 것과 같은 이유입니다. 다음 자막을 침범하면 거기서 끊습니다.
...
조건        길이(초)          shift              align
                        시작 /    끝        시작 /    끝
short          36.2   0.400 / 0.400     0.746 / 3.097
dense          85.1   0.030 / 0.030     0.809 / 2.637
fast           74.2   0.500 / 0.500     0.765 / 1.630
```

**끝 시각 편향이 확인됐습니다.** shift는 시작과 끝 오차가 **정확히 같습니다**(0.400/0.400, 0.030/0.030, 0.500/0.500). 원래 자막 길이를 그대로 옮기니 끝 오차가 시작 오차와 같아질 수밖에 없습니다. align은 끝을 음성에서 다시 찾아 3.097 / 2.637 / 1.630초를 받았습니다. **끝으로 견준 것이 공정하지 않았다는 것이 숫자로 드러났습니다.**

**다만 `keep_duration`은 이 실행에 들어가 있지 않았습니다.** align의 끝 오차가 앞 실행과 소수점까지 같은 것이 단서였습니다. 편집 스크립트가 중간에 멈춰 **코드는 빠지고 문서만 들어갔고**, 측정값이 그대로라 CI도 알아채지 못했습니다. 구현을 다시 넣고 `tests/test_subtitle_realign.py`로 고정했습니다(막아 보고 실제로 실패하는 것을 확인했습니다). 그 파일을 따로 만든 이유는 `test_subtitle_sync.py`가 ffsubsync가 없으면 통째로 건너뛰어서, 거기 두면 CI에서 이 검사도 안 돌기 때문입니다.

### 최종 (CI run #288, `keep_duration` 적용)

```
조건        길이(초)          shift              align
                        시작 /    끝        시작 /    끝
short          36.2   0.400 / 0.400     0.746 / 0.746
dense          85.1   0.030 / 0.030     0.809 / 0.809
fast           74.2   0.500 / 0.500     0.765 / 0.765
```

**끝 오차가 1.630~3.097초에서 시작 오차와 같은 값으로 내려왔습니다.** 예측한 그대로입니다. 길이를 지키면 끝 오차는 시작 오차와 같아질 수밖에 없습니다. 끝 시각 편향은 이것으로 없어졌습니다.

**그래도 shift가 이깁니다: 0.030~0.500초 대 0.746~0.809초.** 세 조건 모두입니다. **한국어 낭독 싱크 정확도를 올리지 못했습니다.**

### 이 실행에서 CI가 빨간불이었습니다 — 제 변경 때문이 아닙니다

`전사 품질 검증 (사람 목소리)`가 **CER 54.8%**(한계 20%)로 실패했습니다. 싱크와 무관한 검사입니다.

원인은 **표본이 바뀐 것**입니다. `fetch_korean_speech.py --count 3`이 같은 요청에 다른 행을 돌려줬습니다. 18:28 실행까지는 "지난해 삼 월 김 전 장관의…"였는데 18:40 실행에서는 "몬토 규는 자녀들이…", "참처럼 얕지 않으니…"가 왔습니다. 새 표본은 숫자 표기가 많아(`오 쩜 구 사 시간` ↔ `5.94시간`) CER이 크게 뜁니다. 이 검사의 주석도 "데이터셋 행이 바뀌면 값도 움직인다"고 적어 두었습니다.

제 변경은 싱크·정렬 경로와 행렬 검증 스크립트만 건드립니다. `verify_transcribe.py`도 `fetch_korean_speech.py`도 손대지 않았습니다.

**제안하는 고침(이 PR에서는 하지 않았습니다)**: 전사 검증이 쓰는 행을 고정해야 합니다. 지금은 데이터셋이 주는 대로 받아서, 코드가 아니라 **그날 뽑힌 표본**을 재고 있습니다. `--offset`으로 행을 박고 어느 행을 썼는지 기록에 남기면 됩니다. 한계값을 올리는 것은 검사를 약하게 만드는 일이라 하지 않습니다.

### 마지막 측정 — 자막마다 다르게 어긋난 경우 (진행 중)

위 열 조건에 없던 모양을 만들어 잽니다. `--drift 0.15`는 자막마다 어긋남을 0.15초씩 키웁니다(첫 자막 0초, 둘째 0.15초, 셋째 0.30초…). **이동값이 하나뿐인 통째로 옮기기는 원리상 고칠 수 없는 모양**이고, 강제 정렬을 제안한 이유가 바로 이것입니다.

여기서도 align이 지면 이 길은 닫힙니다. 이기면 "조건에 따라 고르는" 선택지가 생깁니다.

판정은 `--gate none`으로 **끕니다.** drift에서는 shift가 지는 것이 예상되므로 판정을 걸면 예상된 실패로 CI가 빨개집니다. 숫자만 남깁니다.

### 열 조건 전부 (CI run #288·#296·#302·#308)

| 조건 | 길이(초) | 자막 | shift | align |
|---|---:|---:|---:|---:|
| 잡음 SNR 15dB | 92.8 | 9 | **0.000** | 1.012 |
| 짧은 발화 간격 | 85.1 | 9 | **0.030** | 0.809 |
| 긴 무음 | 128.8 | 9 | **0.030** | 0.921 |
| 0.8배 느린 말 | 116.0 | 9 | **0.030** | 1.066 |
| 반복 구성 긴 영상 | 276.3 | 27 | **0.030** | 0.961 |
| 잡음 SNR 5dB | 92.8 | 9 | **0.040** | 2.628 |
| 8초 음악 도입 | 100.8 | 9 | **0.040** | 1.012 |
| 음량 -18dB | 92.8 | 9 | **0.070** | 0.931 |
| 짧은 영상 | 36.2 | 3 | **0.400** | 0.746 |
| 1.25배 빠른 말 | 74.2 | 9 | **0.500** | 0.765 |

**열 조건 전부 shift가 이깁니다.** shift는 0.000~0.500초, align은 0.746~2.628초입니다.

**다만 이 표에 없는 것이 있습니다.** 열 조건은 모두 자막이 **한 덩어리로** 어긋난 경우입니다(전체를 +2.5초 밀거나 밀지 않거나). 그건 이동값 하나로 고칠 수 있는 모양이고, 통째로 옮기기가 잘하는 일입니다. **align을 제안한 이유였던 「자막마다 다르게 어긋난 경우」는 이 표에 한 번도 나오지 않습니다.** 그러니 이 표는 "align이 shift가 잘하는 일에서 더 나쁘다"를 보인 것이지 "align이 쓸모없다"를 보인 것이 아닙니다.

### 여섯 조건에서 통째로 옮기기가 모두 이깁니다 (CI run #302)

통째로 옮기기가 더 약해질 만한 조건을 골라 재 봤습니다. 더 약해지지 않았습니다.

```
조건            길이(초)     shift      align
long_pauses      128.8      0.030      0.921
slow             116.0      0.030      1.066
noise_5db         92.8      0.040      2.628
```

앞선 세 조건과 합치면 **여섯 조건 모두 shift가 이깁니다.**

| | shift | align |
|---|---|---|
| 짧은 영상 36.2초 | **0.400** | 0.746 |
| 짧은 발화 간격 85.1초 | **0.030** | 0.809 |
| 1.25배 빠른 말 74.2초 | **0.500** | 0.765 |
| 긴 무음 128.8초 | **0.030** | 0.921 |
| 0.8배 느린 말 116.0초 | **0.030** | 1.066 |
| 잡음 SNR 5dB 92.8초 | **0.040** | 2.628 |

"shift가 조건에 따라 흔들리니 어딘가에서 뒤집힐 수 있다"던 여지도 닫혔습니다. shift의 최악은 짧은 영상 0.400초와 빠른 말 0.500초인데, 그 두 조건에서도 align은 0.746·0.765로 더 나쁩니다. **잡음에서는 align이 2.628초까지 벌어집니다.**

**한국어 낭독 싱크 정확도를 올리지 못했습니다. 최선은 통째로 옮기기의 0.50초이고, 기본값은 그대로입니다.**

### 스냅 가설은 기각됐습니다 (CI run #296)

```
조건        길이(초)     shift        align    align_nosnap
short          36.2      0.400        0.746        0.670
dense          85.1      0.030        0.809        0.831
fast           74.2      0.500        0.765        0.765
```

`snap_starts`를 끄면 오차가 **-0.076 / +0.022 / 0.000초** 움직였습니다. 하나는 조금 좋아지고 하나는 조금 나빠졌습니다. **0.75초는 스냅 때문이 아닙니다.**

값이 세 조건 중 둘에서 실제로 달라진 것(0.746→0.670, 0.809→0.831)이 스위치가 먹혔다는 증거입니다. `keep_duration` 때처럼 코드가 빠진 경우와 구분됩니다.

그러므로 **남은 0.75초는 강제 정렬 자체의 오차**입니다. 우리가 붙인 후처리가 아니라 정렬기가 내놓는 단어 시각이 이 자료에서 그만큼 어긋납니다. VAD와 에너지 문턱의 정의 차이로 설명하려던 시도는 실패했습니다.

**결론: 기본값은 `shift`입니다.** 한국어 낭독 싱크 정확도의 최선은 여전히 통째로 옮기기의 0.50초입니다.

### 아래는 기각된 가설의 기록입니다

### 다만 숫자가 한 가지를 더 말합니다 (미검증)

align의 오차가 세 조건에서 **0.746~0.809초로 거의 일정합니다**(폭 0.06초). shift는 0.030~0.500으로 흔들립니다(폭 0.47초). 일정한 오차는 잡음이 아니라 **계통 편차**로 보입니다.

같은 실행의 다른 단계에 단서가 있습니다. 같은 음성에서 **Silero VAD는 발화 시작을 1.86초**로, **에너지 문턱(정답)은 1.08초**로 잡습니다 — **차이 0.78초.** align의 오차 범위와 같습니다. `align_text`는 `snap_starts`로 자막 시작을 VAD 발화 시작에 맞추므로, align의 시작은 VAD 정의를 따르고 정답은 에너지 문턱 정의를 따릅니다.

shift는 이 차이가 **상쇄됩니다.** 상호상관은 전체 무늬를 맞추므로 두 쪽이 같은 만큼 치우쳐 있으면 서로 지워집니다. 강제 정렬에는 그런 상쇄가 없어 절대 시작 시각을 그대로 내놓습니다.

**이것을 근거로 align이 사실은 더 낫다고 주장하지 않습니다.** 끝 시각에서 한 번 겪었듯이, 지면 정답을 의심하는 식으로 계속 옮겨 가면 스스로를 속입니다. 확인하려면 **정답 정의를 바꾸지 말고** `snap_starts` 없이 align을 한 번 재 보면 됩니다. 그때도 0.75초가 남으면 계통 편차 가설이 틀린 것입니다.


### 남은 작업

- 기본값은 **`shift`**로 정해졌습니다. align은 고를 수 있게 남겨 두되 화면·API 문서·설정 주석에 **측정에서 졌다는 사실**을 적었습니다. 고를 수 있다는 것과 권한다는 것은 다릅니다.
- **다음에 볼 곳**: align의 자막 경계입니다. 전체 위치는 0.1초 안으로 맞히므로 `cues_for_lines`가 단어를 줄에 묶는 방식이나 `snap_starts`의 VAD 스냅이 의심됩니다. 고치면 같은 명령으로 다시 재면 됩니다.
- 나머지 일곱 조건(긴 영상·무음·느린 말·음량·잡음·음악)은 아직 align으로 재지 않았습니다. 잡의 시간 예산 때문에 세 가지만 돌립니다. `--variants`로 고를 수 있습니다.
- 통째로 옮기기의 남은 오차(빠른 말 0.50초, 짧은 영상 0.40초)는 그대로입니다.

...
**아직 한 번도 재지 못했습니다.** 세 번 연달아 재기 전에 죽었습니다. 세 번째는 이것이었습니다:
...
torchaudio는 분리 결과를 **32비트 실수** WAV로 씁니다. 표준 `wave`는 정수만 읽습니다. 재는 쪽이 16비트만 읽게 되어 있던 것이 잘못입니다. soundfile이 있으면 그쪽으로 읽습니다. 실수 WAV와 정수 WAV를 같은 신호로 만들어 읽어 보니 최댓값·주파수 분포가 같았습니다(로컬 확인). 두 숫자는 다음 실행이 줍니다.

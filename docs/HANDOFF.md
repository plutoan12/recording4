...
폭만 재면 이 실패는 **안 보입니다.** 두 줄이 세 줄이 되면 자막이 다른 자리에서 끊기고 화면 아래로 더 내려오는데, 줄당 폭은 오히려 줄어듭니다. 통과처럼 보입니다.
...
- **세로 위치는 아직 안 봅니다.** 줄이 늘면 자막이 화면 아래로 내려오는데 안전 여백을 벗어나는지는 모릅니다.
...
- `pytest -q`: **320 통과 / 5 skip**(SQLite, providers 설치 시). 새 테스트 7개.

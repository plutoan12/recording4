...
### 지금 돌고 있는 측정 — 어디서 뒤집히는가

자막당 어긋남을 **0 / 0.15 / 0.3 / 0.45 / 0.6초**로 올리며 한 조건에서 잽니다. `--drift`가 쉼표 목록을 받게 고쳤습니다. 값마다 따로 돌리면 소리를 매번 새로 만들어 느리고, 같은 소리로 잰 값인지도 흐려집니다.

조건은 **8초 음악 도입(music_intro)** 하나입니다. 앞에 8초 여유가 있어 보정값이 자막을 원본 시작 앞으로 밀어내지 않습니다. 여유가 없는 조건에서는 거부부터 나서 두 방법의 오차를 견줄 수가 없습니다(위 표의 「거부」 두 줄).

판정은 그대로 `--gate none`입니다. 어긋남이 큰 쪽에서 통째로 옮기기가 지는 것은 예상된 결과이고, 단어 정렬은 아직 기본값이 아닙니다. 예상된 결과로 CI를 빨갛게 만들지 않습니다.
...
- **다음에 볼 곳**: 어긋남이 쌓이는 모양에서 두 방법이 뒤집히는 지점입니다. 위 측정이 CI에서 돌고 있습니다. 뒤집히면 "조건에 따라 고르는" 선택지가 생기고, 뒤집히지 않으면 그 길은 닫힙니다.

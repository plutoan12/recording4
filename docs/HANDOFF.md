...
- **실측이 없습니다.** 이 변경의 CI가 처음 잽니다. 화자 분리가 사람 목소리 둘을 실제로 가르는지부터 봐야 합니다. 겹말용 목소리(zeroth-korean 뒤쪽 문장)가 같은 화자일 수도 있습니다. 그러면 분리가 안 되고 검증이 그렇게 말합니다.
- 운영 경로(`media_tasks`, API, 관리화면)에는 **아직 넣지 않았습니다.** 숫자를 보고 넣습니다. 넣으면 `transcript_segments`에 `overlap` 열이 필요합니다(마이그레이션 0008).
- `partial0`의 상한은 없습니다(보고만). 잰 뒤 `MEASURED_CER`에 넣습니다.
...
- 반복 측정의 실측이 없습니다. 이 변경의 CI가 처음 잽니다. 폭이 얼마인지, 씨앗 고정이 폭을 없애는지 모릅니다.
- 세 번은 폭의 아래쪽 추정입니다. 폭이 크면 횟수를 늘려야 합니다(CI 시간과 맞바꿉니다).

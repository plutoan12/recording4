...
## 2026-09-19: 자막 템플릿과 브라우저 예시 미리보기

- `codex/subtitle-templates`는 PR #20의 `codex/fix-subtitles-and-ci` 위에 쌓은 변경입니다. Claude 공유 체크아웃은 수정하지 않았습니다. PR #20 병합 후 이 변경을 main 기준으로 재검토합니다.
- 5개 ASS 스타일, 인증된 목록/예시 API, JASSUB 미리보기, 한국어 글꼴과 라이선스 고지를 추가했습니다. 편집과 단계별 제작에서 선택한 스타일이 최종 렌더로 전달됩니다.
- 화자 ID 저장/읽기/편집과 번역 원문 메타데이터 보존을 연결했습니다. 상세 사용법과 한계는 [자막 템플릿](SUBTITLE_TEMPLATES.md)을 봅니다.
- 검증: pytest 290 passed / 5 skipped, Ruff, TypeScript·Vite production build. 오프라인 Docker FFmpeg로 5개 MP4 생성·디코드 성공, 원음 PCM 유사도 각 0.999546. PNG에서 한국어 및 원문 병기 표시 확인.
- Mac Chrome의 독립 로컬 테스트 서버에서 로그인, 한국어 기본 미리보기, 병기형으로 전환을 직접 확인했습니다. 발견한 Vite JASSUB worker 경로 및 CommonJS 의존성 오류, 미리보기 폼 배치를 수정했습니다.
- 미리보기는 예시 문장입니다. 실제 업로드 영상 미리보기·단어별 강조는 미구현입니다. 운영 서버에는 아직 배포하지 않았으며 유료 API나 게시 작업은 실행하지 않았습니다.
...
## Codex 검토 지적 수정 (2026-09-19)

- 8e678d5에서 분리한 codex/fix-subtitles-and-ci 브랜치에서 수정. 공유 체크아웃의 미추적 개발 환경 파일은 변경하지 않음.
- 자막만 번역 모드를 스키마·워커·관리화면에 연결. 한국어 기본 선택, 번역 예산 입력, 원본 음성 유지, 번역 수정 후 새 버전 제작 지원. 전사 비어 있음과 자막 모드의 립싱크 요청을 차단함.
- pyannote CI 실행 조건에 샘플 생성과 같은 verify-align 조건을 추가함. 토큰 등록만으로는 검증하지 않으므로 실제 모델 검증은 PR 라벨 verify-align 또는 커밋 표시 [verify-align]도 필요함. 이 기록이 앞선 토큰만 있으면 실행된다는 설명을 대체함.
- 전체 Python 테스트 282 통과/5 skip. 린트·포맷·TypeScript/Vite 빌드 통과. 토큰·라벨·커밋 표시 8개 조합에서 pyannote 실행 시 두 종류 샘플 준비가 반드시 실행되는 것을 확인함.
- 신규 회귀 검증: 번역 예산 차단, 한국어 번역 후 더빙/음성 교체 미호출, 수정 자막의 클립 시작/끝 시각 유지, 빈 대본 및 립싱크 거절. 실제 유료 API·pyannote 모델·영상 공개는 이번 수정 검증에서 실행하지 않았음. 운영 서버 배포와 사용자의 실제 영상 처리는 별도임.

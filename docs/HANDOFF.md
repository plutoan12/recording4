## 2026-09-20 06시대: 새 다국어 실제 대화 평가 입력 고정

- PR #37의 `89ebdd2`에서 Python·웹·워커 이미지·스택 CI 전체 통과 확인. main 통합은 남은 검토 때문에 초안 유지, 운영 재배포 없음.
- `prepare_conversation_evaluation.py` 추가: 네 언어 실제 대화 선언/사람 주석/2명 이상 화자, 고정 평가 구간, 출처·리비전·이용 조건·선정 규칙, 음원·정답 SHA 검사. 중복 음원/ID, 언어 누락, 변경된 평가 정책을 구분. 대사 없는 코호트 lock만 출력하고 다른 기존 lock을 덮어쓰지 않음.
- 이것은 입력 일관성 검사이며 실제 대화/주석의 진위·법적 권한·학습 데이터 독립성 검증이나 품질 측정이 아님. 낭독/합성 자료를 실제 대화 결과로 합산하지 않음. [준비 절차·형식·후속 기준](quality/conversation-evaluation-2026-09-20/PLAN.md).
- 검증: 새 20개 회귀 검사, 전체 Python **517통과/5skip**, Ruff 통과. 합성 테스트 파일로 검사했으며 새 실제 음성 추론·유료 호출·게시 없음.
- Claude 읽기 전용 검토 후 UTF-8·중복 JSON 키·CLI 경로 노출 방어를 추가했습니다. 발화 비율로 주석 완전성을 추정하거나 동일 정답 해시를 무조건 거부하는 제안은 채택하지 않았고 이유를 계획서에 기록했습니다.
- 남은 일: 실제 한·일·중 대화 자료 확보/사람 검수, lock을 연결한 기준·후보 실행기, CJK 경계와 장시간 처리 검토, 50% 기준 목소리 부재와 독립 음성·시각 근거. 기존 11조건 정답/오배정/미확인 3,767/109/513은 변하지 않음.

## 2026-09-20 05시대: PR #38 병합·싱크 버전과 근거 표시 보완

- PR #38은 필요한 CI 전체 통과 후 #34 브랜치에 `51030ee`로 병합했습니다. #34의 이 새 HEAD에서 Python·웹·워커 이미지·스택 CI 전체 통과를 확인하고 원래 대상 `codex/sync-remaining-failures`에 `f7f2a2b`로 병합했습니다. main 통합 PR #37은 초안 유지, 운영 재배포 없음.
- 싱크 중복 요청에 대본 버전 비교 추가. 같은 버전은 기존 작업 재사용, 새 버전은 409로 구분. 원음 경계와 대사 정렬이 불일치하면 `boundary_support=0`으로 표시하며 승인 기준은 바꾸지 않았습니다. 복구 경로의 경계 검사 오류도 고정된 안전 메시지로 반환합니다.
- 화자 분석용 FFmpeg에 `-nostdin`과 600초 제한 추가. 시간 초과·디코딩 실패·실행 파일 부재는 기존처럼 작업 실패로 남기며 빈 분석 성공으로 바꾸지 않습니다. 자막별 부분 복구는 아직 미구현입니다.
- 관련 새 회귀 검사 8개. Python 전체 **497통과/5skip**, Ruff 통과. 모델 입력과 허용 기준이 같아 이전 11/140조건 재추론 없음, 음성 정확도 향상 주장 없음.
- Claude 패치 검토의 의문은 로컬 확인: `transcript()`는 최신 버전의 ORM 행만 반환하며 `func`/`TranscriptSegment`는 여전히 사용합니다. 버전 없는 구형 진행 작업도 종료 전까지 409로 거부하는 것이 의도된 보수적 동작입니다. 첫 검토 출력은 유효한 코드 검토가 아니어서 폐기하고 소스 포함 재검토를 사용했습니다.
- 다음: #37 최신 CI·남은 CJK 경계/장시간 처리 위험 검토. 새 다국어 실제 대화·50% 기준 목소리·독립 음성/시각 검증은 여전히 남습니다. 다른 작업자 폴더와 비공개 runtime은 수정하지 않았습니다.

## 2026-09-20 04시대 후속: main 검토에서 화자 분석 사전조건 수정

- PR #35 병합 완료. PR #34의 새 b6f1525에서도 전체 CI 통과. 다만 누적 main 검토에서 발견한 아래 수정을 먼저 #34 브랜치에 반영한 뒤 병합할 것.
- 원문 언어·단어 정렬 라이브러리 부재를 빈 분석 성공으로 처리하지 않도록 API/워커에서 명확히 거부. 진행 중 화자 분석의 언어/대본 버전과 다른 요청은 409로 구분. 대본 부재 500 지적은 기존 409 방어가 있어 해당하지 않음을 확인.
- Python **489통과/5skip**, Ruff 통과. 동일 조건의 중복 요청은 유지. 실제 모델 재추론·운영 재배포·유료 호출·게시 없음.
- main 통합 초안은 [PR #37](https://github.com/plutoan12/recording4/pull/37). [사전조건 수정 PR #38](https://github.com/plutoan12/recording4/pull/38)을 #34에 먼저 병합하고 #34 최신 CI를 확인할 것. 나머지 검토 과제는 `docs/MAIN_INTEGRATION.md` 참고.
- 작업 폴더 `/Users/an-youwon/Projects/recording4-main-integration`에서 이어갑니다. 원래 폴더의 다른 작업자 변경은 보존했습니다. 자동화 종료 시각 07:14:35 KST 유지.

## 2026-09-20 04:15 KST: 자동화 1회차 — 검증된 변경 병합·main 통합 준비

- PR #35의 최신 커밋 `eb1b32d`에서 Python·웹·워커 이미지·스택 CI 전체 통과를 확인하고, 지정된 #34 브랜치에 merge commit `b6f1525`로 병합했습니다. #34는 이 새 HEAD에서 CI를 다시 확인 중입니다.
- 원래 작업 폴더에 다른 작업자의 단어 자막 렌더링 변경이 있어 수정·stash·checkout하지 않았습니다. 이후 자동화용 분리 작업 폴더: `/Users/an-youwon/Projects/recording4-main-integration`, 브랜치 `codex/main-integration-review`.
- main 대비 누적 변경은 99파일(대부분 품질 보고서 포함), 18커밋입니다. main과 병합 시 충돌 없음과 코드 트리 동일함을 확인했습니다. 새 모델 실험을 운영에 연결하지 않고 별도 main 통합 PR로 준비합니다.
- 범위: 원문 싱크 근거 검증·대본 버전 보호, 단어별 화자 검수와 부분 정렬 보존, CI 샘플 준비 조건, 실험/품질 판정 도구. 운영 코드의 누적 차이는 Claude에 읽기 전용 검토 요청했습니다.
- 운영 서비스 재배포·유료 호출·게시 없음. 이미 검사한 모델/음원은 재실행하지 않았습니다. 109자 오배정과 실제 한·일·중 대화 검증은 계속 미해결입니다.
- 자동화 종료는 **2026-09-20 07:14:35 KST**입니다. 아래 이전 기록의 '최대 8회'보다 이 시간 제한이 우선합니다.

## 2026-09-20: 쉬운 작업부터 검수·판정 자동화 및 Claude 지적 수정 (Codex)

- `automate_speaker_review.py`: 기존 결과의 동일 SHA/분모·조건별 정답 비감소/오배정 비증가·전체 정답 증가 검사와 DiCoW 검수 후보 생성, 수치 요약을 한 명령으로 실행. 새 추론/운영 배포/게시를 실행하는 명령은 아님.
- 실제 실행: 품질 판정은 추가 정답 없음으로 거부, 검수 후보 25% 12단어+50% 36단어, 실제 화자 변경 0. 후보는 독립 목소리 근거가 아니며 자동 적용 금지. 기존 109자 오배정은 그대로.
- Claude 읽기 전용 검토에서 확인한 크롭 패딩 마스크, 무효 단계 조합, 불완전 시각의 재검사, 거부 이유 누락, 결과 덮어쓰기, glibc 외 실행 실패를 수정. 기존 16추론은 마스크 범위를 넘지 않아 과거 결과 영향 없음. 11개 fixture PCM 재생성 일치 확인. 통제 검사가 정답 문구·정답 발화 경계를 입력으로 사용한다는 한계를 명시.
- OpenCC 1.4.2 간체 출력 별도 실측: 기존 중국어 Whisper 오류 20→5/121, DiCoW 44→20/121. 선택에 쓰지 않은 새 3개 낭독에서도 Whisper 31→4/104. raw CER은 보존, 운영에 간체 강제 적용하지 않음.
- Python 전체 **486통과/5skip**, 변경 파일 Ruff 통과. 운영 환경과 유료 호출·게시 변경 없음. [보고서/재현](quality/speaker-automation-2026-09-20/REPORT.md).
- 후속 Claude 검토도 반영: 누락/잘린 대상별 결과, 미등록 조건 ID, 기존 증거 중간 덮어쓰기 차단. 겹말 4건을 새로 추론해 정상 종료와 같은 CER 확인, 최신 48후보 요약 저장.
- 이 대화의 `recording4` 후속 자동화는 1시간마다 최대 8회 진행하도록 활성화. 다음 실행은 PR #35 최신 CI/검토 확인 → #35를 #34 브랜치로 병합 → #34 검사 후 원래 대상 브랜치로 병합 → main 통합 준비 순서. 완료/필수 인증 장애면 자동화 중지.
- PR #34 CI 전체 통과 확인. 후속 변경 검토/CI를 거쳐 상위 브랜치에 병합할 것. main까지 기존 여러 PR이 누적되어 있으므로 실험 PR 병합을 main 배포 완료로 표현하지 말 것.

## 2026-09-20: 분리 실패 계측·기존 배정 재검사·DiCoW 실제 비교 (Codex)

- 분리 실행 여부, 채널 대응/혼합 진단, 목소리 점수와 화자 간 차이, ASR 일치율, 정렬 탈락을 분리 기록. 25% 두 조건의 16개 채널/대사 후보 모두 목소리 문턱 미달. 50% 두 조건은 기준 목소리 부재로 모델 실행 전 중단. 과거 '추가 회복 0' 설명을 정정.
- 명시적 `--reconsider`에서 실제 겹말의 기존 배정도 검토. 목소리·독립 ASR·두 정렬 합의를 요구하고 이전 화자와 검수 표시 보존. 승인된 1·2단계에서 다시 검사한 11조건은 **3,767/109/513**(정답/오배정/미확인), 기존 배정 변경 0. 추가 정답 개선은 확인하지 못함.
- DiCoW 고정 v3_2를 별도 오프라인 컨테이너에서 CPU INT8·CTC 가중치 0으로 실제 실행. 새 낭독 12개+겹말 두 조건×두 화자=16추론 완료. 새 낭독 CER ko 11.70→5.85%, en 0.47→0.47%, ja 18.11→8.66%, zh 16.53→36.36%. 중국어 번체/간체 차이도 같은 기준으로 오류 계산. 네 언어 공통 모델 교체 미채택.
- 같은 예측 구간의 Whisper small 오류 25% 80자→DiCoW 6자, 50% 155자→126자. SepFormer+small은 25% 110자, 50% 기준 목소리 부재로 미실행.
- DiCoW 겹말 대사 CER: 25%에서 6/399(1.50%), 50%에서 126/399(31.58%). 이 지표를 기존 화자 문자 배정 점수로 대체하지 않음.
- 새 실제 영어 회의 AMI 120초: 사람 주석 화자 4명, 예측 2명, DER 34.49%(collar 0·겹말 포함). 실제 한·일·중 대화·대규모 새 화자 검증은 남음. FLEURS는 언어별 3개 낭독이며 모델 학습에 없던 화자라는 보장 없음.
- Python 전체 **466 통과/5 skip**, 변경 파일 Ruff 통과. 원음·유료 호출·게시 변경 없음. 새 실험은 운영 기본에 연결하지 않음. 비밀·원시 음원/전사·모델은 비공개 runtime에만 보관. [실측·재현·제한](quality/speaker-diagnostics-2026-09-20/REPORT.md), PR #34에 저장.

## 2026-09-20: 화자 정확도 개선 1→2→3→4 구현·실측 (Codex)

- 1단계 정상 단어 보존, 2단계 순차 교대/실제 겹말 구분을 기본 적용. 원문 문자 분모 유지, 시각 없는 단어는 UI에 `시각 미확인` 표시. 겹말 검수·기존 버전 보존 유지.
- 같은 11조건 4,389자에서 변경 전 3,069/109/1,211(정답/오배정/미확인) → 1·2단계 **3,767/109/513**. 정답 비율 **69.9%→85.8%**, 오배정 비증가. 2단계 단독 추가 점수 향상은 없음.
- 3단계 분석 사본 stable-ts + WhisperX 음소 정렬: 3,769/112/508. 오배정 3자 증가로 기본 적용 제외. 4단계 전체 경로는 3단계 대비 추가 회복 없음. 후속 계측 정정: 25% 겹말은 실제 분리 후 목소리 검증 탈락, 50% 겹말은 기준 목소리 부재로 분리 전에 중단.
- 3·4단계는 `worker.speaker_recovery`와 `scripts/verify_overlap_review.py --stage 3|4` 실험 경로로 보관. 운영 Celery는 호출하지 않음. 당시 경로는 기존 배정을 유지하며 미확인만 회복 후보로 평가. 후속 `--reconsider` 실험 옵션은 아래 보고서 참고.
- Mac API·워커·웹 재빌드 및 배포 완료. 실행 작업 0 확인 후 교체, 서비스 healthy, 배포된 화자/회복 모듈 SHA 일치 및 정상/미확인/정상 단어 보존 스모크 통과. 후속 [PR #34](https://github.com/plutoan12/recording4/pull/34)에 저장.
- 기준 브랜치의 PR #31 병합에서 추가된 중복 NumPy 핀(2.2.6과 2.2.2)을 정리하여 기존 검증 버전 2.2.6 하나를 유지. PR #30은 이미 기준 브랜치에 병합되어 새 후속 PR로 저장.
- Python 전체 **461통과/5skip**, Ruff·TypeScript/Vite 통과. 원본 WAV SHA 유지, 유료 호출·추가 게시 없음. 단계별 수치와 한계는 [보고서](quality/speaker-recovery-2026-09-20/REPORT.md). 네 언어 일반화, 새 화자/현장 소음, 실제 주관적 청취는 이번 검증에 포함되지 않음.

## 2026-09-20: 단어별 화자 배정·겹말 검수 구현 및 재검증 (Codex)

- 문장 최장 겹침 강제 배정 대신 단어별 원문 정렬을 연결. 복수 후보는 `복수 화자`, 근거 부족은 미확인으로 남기고 겹말/정렬 실패/복수 배정은 `검수 필요` 표시. 관리화면에 원문 언어 선택·분석 버튼·단어별 검수 패널 추가.
- 문장 문구·시각 보존, 새 대본 버전에 단어 결과 연결. 대본 버전/언어 고정, 처리 중 대본 변경 시 저장 차단, 구버전 리뷰 숨김. 복수 화자는 실제 화자 수나 단일 배정 완료 수에서 제외.
- 같은 11개 WAV SHA 확인 후 재검증. 총 4,389자: 오배정 344→109, 정답 4,045→3,069, 미확인 0→1,211. **오배정 감소와 자동 정답 범위 감소를 같이 보고할 것. 전체 정확도 개선/겹말 해결 완료 아님.** 0 dB에서 이전 정답 문장 188자씩 미확인으로 남는 두 조건이 있음.
- 50% 겹말 오배정 172→56자, 잡음 포함 172→53자. 두 조건의 모든 문장은 검수 대상으로 표시됨. [재현·전체 비교·한계](quality/overlap-review-2026-09-20/REPORT.md).
- Python 452통과/5조건부 skip(포트 제한 4개는 권한 환경 재실행), 후속 API 12개·Ruff·웹 빌드 통과. API·워커·웹 최종 이미지 반영, API/워커 healthy, 배포 소스 해시 일치와 단어 배정 스모크 확인. 브라우저 실제 화면 직접 검증은 하지 않음.
- 새 유료 호출·게시·원본 수정 없음. 원본 검증 음원 및 상세 단어 결과는 비공개 runtime 보관. 주관적 청취는 이전 미완료 상태 유지.

## 2026-09-20: 소음·겹말 11조건 실측 및 청취 제한 확인 (Codex)

- 실제 두 화자 음성을 혼합한 11조건 완료. 백색잡음 20/10/0 dB × 2시드에서 문장 화자 배정 모두 4/4. 50% 겹말과 같은 조건+잡음에서는 각각 2/4로 하락. 전 조건 화자 수 2명이므로 화자 수만으로 성공 판정하면 안 됨.
- 겹말 재현율은 깨끗한 개별 음성의 20ms 에너지 기준으로 산출한 대리 지표. 사람 주석 DER이나 현장 소음 일반 성능 아님. [전체 수치·재현·한계](quality/diarization-stress-2026-09-20/REPORT.md).
- 기존 실제 더빙 출력: 클리핑 0, -23.98 LUFS / true peak -4.83 dBTP, 끝부분 무음 약 2.39초. 오디오 입력 도구가 미지원으로 거부하여 주관적 청취는 **미완료**. 파일 패널 열기를 요청했으나 사용자 청취 완료를 뜻하지 않음.
- 재현 스크립트와 지표 테스트 2개 추가. 원본 SHA 보존, 유료 호출·게시 없음. 이전 HF 연결 수정 커밋 `1a5226b`의 CI는 pyannote 포함 전체 통과(run 35454332087). 이번 변경은 검증 도구·보고서이며 운영 코드 변경 없음.

## 2026-09-20: HF 연결·실제 자동 화자 수 검증 완료 (Codex)

- 사용자 모델 접근 동의 및 토큰 갱신 완료 후 제한 토큰을 로컬 비공개 설정(0600)과 `plutoan12/recording4` Actions `HF_TOKEN`에 저장. 진행 중 작업 0 확인, 검증 음성 보존 후 API·워커·디스패처·모니터 재생성. API/워커 healthy, 토큰 로딩 확인.
- 실제 LibriSpeech 두 화자·네 문장 34.40초 표본에서 pyannote community-1 실행: 화자 수 힌트 없이 2명 자동 검출, 문장 화자 할당 4/4 정답. 표본 1개 결과이며 소음·겹말·다국어 일반 성능 및 DER은 미검증.
- 비밀값·음성·원시 로그는 커밋하지 않음. [실측 보고서](quality/live-services-2026-09-20/REPORT.md)에 한계 포함. HF 연결 후 원격 CI에서 표본 준비 조건 오류(`/audio/expected.json` 없음)를 발견. 준비 단계를 분리해 HF 토큰만 있어도 생성하도록 수정. 최신 CI 결과는 별도 확인.
- 기존 미완료: 주관적 음질 청취, 중국어 STT CER 34.3% 개선, 사용자 영상 스포츠 인명/중계 대사 검수. 이미 비공개 게시한 결과를 중복 업로드하지 말 것.
...
- 음량·역전사·전체 디코딩·자막 프레임 검사 완료. 실제 청취는 사용자 응답 대기. HF 연결·pyannote 추론은 위 후속 기록에서 완료. 사용자 영상의 인명/중계 대사 언어 검수 필요.
...
- **결과 112/140 통과, 28거부, 기준 초과 성공 처리 0.** 3.94초 음악 오판 3개는 거부, 느린 영어 3개는 복구. **이전 통과 19개도 거부**되므로 전부 해결/회귀 없음/전체 정확도 개선이라고 설명하면 안 됩니다.
- 기준 0.5초·원음/문구/자막 길이 보존 유지. 모든 모드에서 근거 부족·충돌을 차단하며, 원문 언어가 없는 경우 UI에서 선택하도록 안내합니다. 프레임률 변경 모드는 검증 불가로 거부합니다.
- 원문 버전/언어 고정(구버전 대기 작업도 실행 시 고정), 처리 중 대본 수정 시 결과 저장 중단. 원문 보정 후 새 번역 작업이 시각을 복사함을 검사. 기존 번역·영상·승인 버전 자동 수정 없음.
- Python 445통과·5skip, Ruff·TypeScript·Vite 통과. 실제 비발화 WAV 9/9 거부, 잡음 제거 사본 길이·원본 SHA 불변 확인. Mac 잠금으로 브라우저 실화면 재검증은 못했습니다.
- API·워커·디스패처·모니터·웹 재빌드/재시작 및 전체 healthy. 배포 소스 해시 일치, 캐시 없이 8건 예상 동작 확인(음악 3거부·느린 영어 3복구·일본어 2통과), 원본 SHA 유지.
- [수치·변화·재현·한계](quality/sync-source-verification-2026-09-20/REPORT.md). 무료 로컬 모델/FFmpeg만 사용, 유료 호출·YouTube 게시 없음.

## 2026-09-19: 남은 싱크 실패 조건 수정 및 추가 표본 검증 (Codex)

- 브랜치 `codex/sync-remaining-failures`, PR #28(`codex/speech-sync-robust`) 위 후속. 원문 말투 유지 정책은 그대로입니다.
- `quiet`/`long_cues` 일정 이동 보정에 분산된 원음 경계 3개 이상의 합의와 제한된 미세 보정을 추가. 실패 시 강한 정규화 재시도도 원음 경계 근거를 요구합니다. 탐색 한계·일정한 잡음 방어 추가. standard는 유지합니다.
- 기존 영어 8개·일본어 2개 실패 수정: 네 언어 기존 조건 **80/80** 통과. 최대 오차 en 0.43/ja 0.10/ko 0.50/zh 0.05초.
- 기존과 겹치지 않는 6개씩의 추가 en/ja 음원: **영어 3/30→18/30, 일본어 29/30→30/30**, 기존 통과 사례의 실패 전환 없음. 새 영어 느린 말·잡음·음악 **12개 실패는 남음**. 특히 음악 3개는 기존처럼 잘못된 이동값을 성공으로 반환하므로 검수가 필요합니다. 전체 영상 자동 품질 보장으로 보고하지 마세요.
- Python **433 통과·5 skip**, Ruff 통과. 무음·일정한 톤·잡음 WAV 9/9 거부, 인덱스 최적화 결과 동등성 3,240개 확인. UI 호출 계약 변경 없음, 이번 브라우저 재검증은 하지 않음.
- 운영 API·워커·디스패처·모니터 재빌드/재시작, 전체 healthy. 배포 소스 해시 일치 및 실패했던 저음량 en/ja MP4 8/8 재검증, 원음 SHA·원문 보존 확인.
- [수치·출처·재현·한계](quality/sync-boundaries-2026-09-19/REPORT.md). 영상·대본·비밀 파일은 커밋하지 않음. 추가 표본 원본은 `/tmp/r4-holdout-preserved-en`, `/tmp/r4-holdout-preserved-ja`에 보관했습니다.

## 2026-09-19: 저음량·긴 자막 보정 선택 및 무음 방어 (Codex)

- API·워커·디스패처·모니터·웹 재빌드/재시작 및 healthy 확인. 운영 워커에서 en quiet/ja long_cues MP4 4개 보정 재검증 통과, 원음 SHA 유지.
- 브랜치 `codex/speech-sync-robust`, PR #27 후속. 관리화면에 기본/작은 음량/긴 문장 보정 선택을 추가해 API·워커까지 연결. 기본은 standard 유지, 새 방식은 명시적 선택입니다.
- 실제 동일 표본: 영어 quiet **12/20**(이전 0), 일본어 long_cues **18/20**(이전 0). 한국어·중국어 standard 재검증 각각 **20/20**. 네 언어 자동 선택/전부 통과가 아닙니다.
- 실제 무음이 ffsubsync 성공으로 처리되는 버그 발견: 발화/비발화가 각각 0.5초 미만인 상수·빈 마스크를 거부하고 원래 대본 보존. 분석용 음원만 조정하며 원음 SHA/길이 보존 확인.
- Python 411 통과·5 skip, Ruff·TypeScript·Vite 통과. 실제 Chrome 선택 메뉴 확인. [결과·제한·재현](quality/sync-profiles-2026-09-19/REPORT.md).
- 남은 작업: 영어 느린 말/추가 저음량/잡음/음악 8조건, 일본어 추가 저음량 2조건, 독립적인 사람 검증 기준 및 현장 영상 검증.

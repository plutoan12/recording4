...
제작 방식에서 **자막만 번역·원래 음성 유지**를 선택하면 대상 언어가 한국어 `ko`로 설정됩니다. 작업 예산을 입력하고 제작을 시작하면 전사 → Google 번역 → 번역 자막 영상 → 검수 대기로 진행됩니다. 원래 대사와 배경음을 유지하며 출력은 기존 AAC 인코딩을 사용합니다. ElevenLabs·음성 합성·립싱크는 호출하지 않습니다.

API는 `workflow.audio_mode="subtitles"`를 사용합니다. 기존 `original`은 번역 없는 원어 자막, `dub`은 번역·더빙 동작을 유지합니다. 번역에는 기존 월/건별 예산과 유료 처리 설정이 적용됩니다. 예산 0은 실제 유료 호출 전에 차단됩니다. 번역 검수에서 수정한 문장은 **수정 자막으로 새 버전 제작**으로 합성하며 다시 승인해야 합니다.

...
3. 원어 유지 또는 번역·더빙을 선택합니다. 더빙은 음성 ID, 작업 예산, 월 공통 예산이 필요합니다. 립싱크는 선택 사항입니다.
   - **문맥 배치**(자막만 번역일 때)를 켜면 한 문장으로 잘린 자막을 합쳐서 번역하고 다시 나눕니다. 번역 요금은 그대로입니다. **어색한 자막만 LLM으로 다시 번역**은 추가 요금이 들고 서버에 LLM 키·모델 이름·단가 상한이 있어야 켜집니다. 둘 다 [번역 품질 확인·용어집](TRANSLATION_CHECK.md)에 방식과 한계를 적었습니다.

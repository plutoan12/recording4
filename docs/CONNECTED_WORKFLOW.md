...
3. 원어 유지 또는 번역·더빙을 선택합니다. 더빙은 음성 ID, 작업 예산, 월 공통 예산이 필요합니다. 립싱크는 선택 사항입니다.
   - **문맥 배치**(자막만 번역일 때)와 **어색한 자막만 LLM으로 다시 번역**은 **기본으로 켜져 있습니다.** 앞쪽은 한 문장으로 잘린 자막을 합쳐 번역하고 다시 나누며 번역 요금이 그대로입니다. 뒤쪽은 서버에 LLM 키·모델 이름·단가 상한이 있어야 실제로 돌고(없으면 조용히 건너뜁니다) 그때는 추가 요금이 작업 예산 상한에 포함됩니다. 필요 없으면 양식에서 끄세요. 방식과 한계는 [번역 품질 확인·용어집](TRANSLATION_CHECK.md)에 있습니다.
   - 채널 이름·제품명처럼 번역 표기가 매번 같아야 하는 말이 있으면 **작업을 만들기 전에** 화면 아래 **용어집**에 적습니다. 바꾼 용어는 그다음 번역 단계부터 적용되고 이미 끝난 번역은 바뀌지 않습니다. 원문 언어를 지정하지 않으면(자동 감지) 적용되지 않습니다. 방식과 한계는 [번역 품질 확인·용어집](TRANSLATION_CHECK.md#용어집-2026-09-21-추가)에 있습니다.
...
원어 경로는 번역을 하지 않습니다. 더빙 경로는 기본적으로 원래 음성을 통째로 교체하므로 배경음도 사라집니다. `R4_BACKGROUND_AUDIO_ENABLED`를 켜면 목소리만 뺀 소리를 대사 아래에 깝니다. 분리를 못 하면(모델 번들이 없는 torchaudio 등) 더빙은 그대로 진행하고 배경음만 빠지며, 그 이유를 로그에 남깁니다. 최대 15% 속도 조정으로 다음 대사 시작 전까지 음성을 맞추며, 이 범위를 넘으면 대본 수정을 요청합니다. 이 수치는 초기 구현 기준이며 실제 음질 평가는 별도입니다. 자막 종료는 합성된 음성 길이에 맞춥니다.

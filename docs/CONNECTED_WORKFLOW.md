...
이미 자막 **파일**이 있으면 편집기의 **자막 파일 가져오기**에 넣습니다(SRT·WebVTT·ASS). `POST /source-assets/{id}/transcript/import`가 대본 새 버전으로 저장합니다. 시각은 파일에 적힌 그대로 쓰며 원본 음성과 맞는지는 확인하지 않습니다. 다른 판본에서 만든 자막이면 통째로 어긋날 수 있으니 들인 뒤 확인하세요. 글자가 없거나 시간이 올바르지 않은 자막은 빼고 무엇을 왜 뺐는지 함께 알려 줍니다.

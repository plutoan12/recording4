...
이미 자막 **파일**이 있으면 편집기의 **자막 파일 가져오기**에 넣습니다(SRT·WebVTT·ASS). `POST /source-assets/{id}/transcript/import`가 대본 새 버전으로 저장합니다. 시각은 파일에 적힌 그대로 쓰며 원본 음성과 맞는지는 확인하지 않습니다. 다른 판본에서 만든 자막이면 통째로 어긋날 수 있으니 들인 뒤 확인하세요. 글자가 없거나 시간이 올바르지 않은 자막은 빼고 무엇을 왜 뺐는지 함께 알려 줍니다. 파일은 바이트 그대로 보내고 서버가 글자를 찾습니다. UTF-8이나 BOM이 있는 파일은 그대로 읽고, 그 밖의 인코딩(한국어 자막에 흔한 CP949 등)은 **추측하지 않습니다.** 후보마다 첫 자막이 어떻게 보이는지 보여 주므로 **글자가 제대로 보이는 것**을 고르면 됩니다.

들인 자막이 원본과 통째로 어긋나면 **자막 싱크 보정**을 누릅니다. `POST /source-assets/{id}/transcript/sync`가 ffsubsync로 시각을 옮긴 새 대본 버전을 만듭니다. 글자는 건드리지 않고, 얼마나 옮겼는지가 작업 목록에 나옵니다. 기존 버전은 남으므로 마음에 들지 않으면 되돌리면 됩니다.
...
## 일본어·중국어·한국어·영어 자막만 번역

1. 원본을 등록하고 편집기의 대본을 확인합니다. 필요하면 **대본 정렬**로 원문을 원음에 맞춥니다. 번역문을 다른 언어의 음성에 직접 정렬하지 않습니다.
2. 단계별 제작에서 **자막만 번역 · 원음 유지**를 고릅니다. 원본 언어는 한국어(ko), 영어(en), 일본어(ja), 중국어(zh), 자동 감지 중에서 고르고 대상 언어를 선택합니다. 중국어는 현재 zh 한 가지이며 간체·번체 별도 선택은 없습니다.
3. 번역 API는 기존 유료 처리 설정과 월·작업 예산 검사를 따릅니다. 작업 예산을 입력합니다(현재 화면 상한 1달러). 더빙 키와 음성 ID는 필요하지 않습니다.
4. 원문 구간의 시작·종료 시간을 유지해 번역합니다. 원본 음성으로 영상을 만들고, 번역 검수에서 수정하면 새로운 작업·결과물로 만들어 다시 승인합니다.
5. SRT·VTT를 내려받거나 영상에 굽기/YouTube 트랙만 중 하나를 선택합니다. 줄 나눔·긴 자막 분할은 기존 표시 규칙을 따릅니다.

한 작업은 한 대상 언어입니다. 여러 언어 결과가 필요하면 원본을 재사용해 언어별 작업을 만듭니다. 12가지 언어 간 방향은 공급자 대역으로 연결·시간 보존·내보내기를 검증했으며 실제 번역 품질 검증과 사람 목소리 자동 싱크 정확도 개선은 별도입니다.

## 싱크 보정 방식 선택

편집기의 **자막 싱크 보정**을 열어 방식을 고릅니다. 기본은 기존 방식이고, 목소리가 작거나 음량 차이가 크면 **작은 음량·음량 차이 보정**, 한 자막이 긴 문장 전체를 포함하면 **긴 문장 자막 보정**을 시험할 수 있습니다. 분석용 사본만 음량을 조정하며 최종 원음과 자막 글자는 유지합니다. 완료 후 **대본 다시 읽기**로 새 버전을 확인하세요. 앞·중간·끝 싱크를 반드시 확인하고, 실패하면 기존 대본이 유지됩니다. 같은 원본의 다른 보정이 진행 중이면 완료 후 다시 요청하세요. 새 방식도 모든 영상에 더 정확한 것은 아니며 [검증 범위](quality/sync-profiles-2026-09-19/REPORT.md)에 실패 조건을 기록했습니다.

### 원문 대사로 싱크 검증

보정기의 성공 표시만으로 새 대본을 저장하지 않습니다. 원음 경계 근거가 부족하면 원문 대사를 로컬 모델로 정렬해 이동값을 비교합니다. 필요한 경우 분석 사본만 잡음 제거·음량 보정해 재시도합니다. 결과가 충돌하거나 근거가 부족하면 보정을 적용하지 않습니다.

편집기에서 **원문 음성 언어**를 고르세요. 영어 음성이면 영어를 선택하고 영어 원문 대본을 사용합니다. 한국어 번역문을 원문 대본으로 가져오지 마세요. 보정 후 새 번역 작업을 만들면 원문 시각을 한국어 자막으로 복사합니다. 기존 번역 결과·영상·승인 버전은 자동으로 수정하지 않습니다. 원본에 언어가 등록돼 있으면 그 언어를 사용할 수 있습니다.

요청한 원문 버전과 언어를 작업에 고정합니다. 검사 도중 원문을 수정하면 오래된 보정 결과는 저장하지 않으므로 새 대본으로 다시 요청하세요. 정밀 검증은 시간이 더 걸리며, 짧거나 잡음이 큰 영상은 기존 방식보다 자주 거부될 수 있습니다. [140조건 검사와 한계](quality/sync-source-verification-2026-09-20/REPORT.md)를 확인하세요.

## 화자·겹말 검수

숏폼 편집 화면의 **화자·겹말 검수**에서 원문 언어를 선택하고 **저장된 대본 화자 분석**을 실행합니다. 저장된 대본을 분석하므로 편집 중인 문구는 먼저 저장해야 합니다. 결과에는 대본 버전, 구간별 화자, 단어별 시각과 화자, 검수 필요 여부가 표시됩니다. 겹말이나 경계가 모호하면 복수 화자, 단어 시각을 확인하지 못하면 미확인으로 남깁니다. 해당 구간을 원음과 대조해 검수하세요. 후속 대본 수정 시 과거 분석은 표시하지 않으며 새 버전으로 다시 요청해야 합니다.


### 부분 화자 정렬과 실험 경로

화자 분석은 일부 단어가 실패해도 정상 단어의 결과를 보존합니다. 누락된 부분은 미확인, 잘못된 시각은 `시각 미확인`으로 표시하며 전체 대본 문구는 유지합니다. 실제 겹말은 계속 검수가 필요합니다. 분석 사본·음소 정렬·음성 분리는 현재 비교 실험 CLI에서만 실행하며 관리화면의 기본 분석에는 적용하지 않습니다. [비교 결과와 실행 방법](quality/speaker-recovery-2026-09-20/REPORT.md).

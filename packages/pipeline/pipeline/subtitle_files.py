"""클립 편집본 자막을 SRT·WebVTT 글자로 만듭니다.

순수 계산만 둡니다. 파일을 읽거나 쓰지 않고 문자열만 돌려줍니다. 파일 형식
변환은 이미 쓰고 있는 pysubs2가 맡습니다(근거는 docs/TECH_DECISIONS.md).

내보내는 자막은 영상에 굽는 자막과 같습니다. 구간 밖 자막을 빼고, 시각을 클립
시작 기준으로 옮기고(`clip_cues`), 같은 표시 규칙으로 줄바꿈·분할합니다
(`apply_rules`). 화면 제목은 자막이 아니라 화면 구성이라 넣지 않습니다. SRT와
WebVTT에는 위치 지정이 없어 제목을 넣으면 첫 자막과 시간이 겹칩니다.
"""
...
def clip_cues_for_output(spec: EditSpec, rules: SubtitleRules = DEFAULT_RULES) -> list[Cue]:
    """영상에 굽는 것과 같은 자막. 시각은 클립 시작을 0초로 둡니다."""
    return apply_rules(clip_cues(spec.cues, spec.start, spec.end), rules)


def subtitle_file(
    spec: EditSpec, subtitle_format: SubtitleFormat, rules: SubtitleRules = DEFAULT_RULES
) -> str:
    """편집본 자막을 SRT 또는 WebVTT 글자로 돌려줍니다. 자막이 없으면 빈 글자입니다."""
    if subtitle_format not in MEDIA_TYPES:
        raise ValueError(f"지원하지 않는 자막 형식입니다: {subtitle_format}")
    cues = clip_cues_for_output(spec, rules)
    if not cues:
        return ""

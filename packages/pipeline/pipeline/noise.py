...
# 사람의 말을 받아쓰고 있습니다.
#
# **한 번 잰 값입니다.** 같은 코드에서 다시 돌렸더니 소음 0dB가 23.1% → 20.9%,
# 5dB가 17.2% → 14.2%로 움직였습니다. 잡음 씨앗을 고정한 뒤에도
# (verify_robust.NOISE_SEED) 소음은 13.4 / 10.4 / 13.4 / 24.6으로, 파일이 고정인
# 겹말 0dB마저 116.4% → 98.5%로 움직였습니다. **전사기 자체가 같은 소리에
# 매번 다르게 답합니다.** 그래서 한 번 잰 값을 그대로 상한의 근거로 쓰면 다음
# 실행이 우연히 넘습니다. 이 표는 `--repeat`로 여러 번 잰 **가장 나쁜 값**으로
# 갱신해야 합니다.
MEASURED_CER = {
    "clean": 0.104,
    "noise20": 0.112,

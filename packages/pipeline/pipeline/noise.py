...
# 사람의 말을 받아쓰고 있습니다.
#
# **잡음 씨앗을 고정한 두 실행(71403b1, f154b3a) 중 큰 쪽입니다.** 한 실행
# 안에서는 세 번 받아써도 글자 하나까지 같았고(폭 0) 디코더 난수 고정도 차이
# 0이었는데, 실행 사이에는 다릅니다(소음 13.4/10.4/13.4/24.6 ↔ 12.7/11.2/14.2/
# 25.4, 겹말 0dB 98.5 ↔ 116.4). 난수가 아니라 러너 CPU가 다른 탓으로 의심하고
# 있습니다(verify_robust --cpu-isa). 씨앗 고정 전 실행의 소음 값(17.2, 23.1)은
# 잡음이 달랐으므로 넣지 않았습니다. 그 값도 아래 상한 안입니다.
MEASURED_CER = {
    "clean": 0.104,
    "noise20": 0.134,
    "noise10": 0.112,
...
# 이 선을 훌쩍 넘습니다. 겹말 0dB는 실행 사이 폭이 18%p라 이 여유가 모자랄
# 수 있습니다. 넘으면 먼저 로그의 CPU 이름을 지난 실행과 견줍니다.
HEADROOM = 0.10
